#!/usr/bin/env python3
"""Read-only lint script for docs/llm_wiki/: 11 checks, punch-list output, exit 0/1."""
...
def collect_insights(wiki_root: Path) -> list[Path]:
    """Return all insight .md files excluding index/archive/trash."""
    entries = wiki_root / "entries"
...
def check_folder_definition_overlap(wiki_root: Path) -> list[str]:
    """[6/9] No two folder definitions in ROOT_INDEX.md have ≥ 0.7 character Jaccard overlap."""
    root_index = wiki_root / "ROOT_INDEX.md"
...
def check_raw_source_reverse(
    ...
    graph_report = wiki_root / "GRAPH_REPORT.md"
    if not graph_report.exists():
        return ["  GRAPH_REPORT.md not found — run scripts/regen_wiki_graph.py to generate it."]
...
def load_global_tags(wiki_root: Path) -> set[str]:
    """Extract active tag names from _global_tags.md."""
    tags_file = wiki_root / "entries" / "_global_tags.md"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Read-only lint for docs/llm_wiki/ — emits a punch list of issues."
    )
    parser.add_argument(
        "--root",
        type=Path,
        default=DEFAULT_ROOT,
        help="Root of the LLM Wiki (default: docs/llm_wiki).",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    # ── Load data ────────────────────────────────────────────────────────────
    insight_paths = collect_insights(wiki_root)
    if not insight_paths:
        print(f"No insight files found under {wiki_root}", file=sys.stderr)
        return 1

    insights = load_all_insights(insight_paths)
    id_set: set[str] = {ins.id for ins in insights if ins.id}
    global_tags = load_global_tags(wiki_root)
    ...
    header = (
        f"{'='*40}\n"
        f"docs/llm_wiki/ lint report — {TODAY} {now_str}\n"
        f"{'='*40}"
    )
    ...
    issues_overlap = check_folder_definition_overlap(wiki_root)
    issues_orphans = check_old_orphans(insights)
    issues_raw_src, skipped_raw = check_raw_source_resolution(insights, wiki_root)
    issues_reverse = check_raw_source_reverse(insights, wiki_root)
    ...
    if not args.quiet:
        ...
        if genuine_errors:
            ...
            out.append("Summary: ✅ All checks passed — LLM Wiki is clean.")
        out.append("=" * 40)
