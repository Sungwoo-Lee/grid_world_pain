#!/usr/bin/env python3
"""Walk docs/llm_wiki/entries/, extract [[id]] wikilinks from bodies, rewrite related: frontmatter."""
...
def process_file(path: Path, dry_run: bool = False) -> bool:
    ...
    # otherwise the backlink IDs injected by regen_wiki_graph.py would pollute
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Rewrite related: frontmatter in wiki entries from [[id]] body tokens."
    )
    parser.add_argument(
        "--normalise-existing",
        action="store_true",
