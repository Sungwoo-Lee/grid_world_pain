...
def evaluate_checkpoint(checkpoint_path, results_dir, config, wandb_run_path=None):
    ...
    # Injury / Injury Params (Backward Compatibility with 'health')
    with_injury = config.get('body.with_injury')
    if with_injury is None:
         with_injury = config.get('body.with_health', False)
    
    # Map old names to new for consistency in this script
    max_injury = config.get('body.max_injury')
    if max_injury is None: max_injury = config.get('body.max_health')
    
    start_injury = config.get('body.start_injury')
    if start_injury is None: start_injury = config.get('body.start_health')
    
    injury_recovery = config.get('body.injury_recovery')
    if injury_recovery is None: injury_recovery = config.get('body.health_recovery')
    
    random_start_injury = config.get('body.random_start_injury')
    if random_start_injury is None: random_start_injury = config.get('body.start_health_random', False)
    
    # Ensure they are not None now
    if with_injury:
         if max_injury is None: raise ValueError("Strict Config: Configuration key 'body.max_injury' (or 'body.max_health') is required.")
         if start_injury is None: raise ValueError("Strict Config: Configuration key 'body.start_injury' (or 'body.start_health') is required.")
         if injury_recovery is None: raise ValueError("Strict Config: Configuration key 'body.injury_recovery' (or 'body.health_recovery') is required.")
    
    # Defaults for non-active
    max_injury = float(max_injury) if max_injury is not None else 1.0
    ...
    if algorithm == "DQN":
        ...
        class CompositeEnv:
            def __init__(self, env, body):
                ...
                self.max_injury = body.max_injury # Standardize on interoceptive terminology
