...
- **Critical config settings.** [docs/environment/CONFIG_CRITICAL_SETTINGS.md](docs/environment/CONFIG_CRITICAL_SETTINGS.md) is the registry of high-impact settings (canonical values + meaning, e.g. `sensory.decay_power`) plus a dated change log. Config-authoring and training-launch agents consult it and keep it updated: any change to a registry setting MUST add a dated change-log entry in the **same commit** (logging protocol), per its Maintenance Contract — this exists because `decay_power` silently drifted 1.0→2.0 inside an unrelated refactor and went unnoticed for months.
