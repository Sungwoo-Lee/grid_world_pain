...
- **Lab GPU spec.** [docs/environment/LAB_NODE_GPU_SPEC.md](docs/environment/LAB_NODE_GPU_SPEC.md) — every lab node (101–114) has exactly **2 GPUs, indices `0` and `1` (there is NO GPU `2`)**, all identical NVIDIA RTX 2080 Ti (11 GB). Consult before assigning a `node:GPU` for any training launch: never request `cuda:2`+; the cluster is homogeneous, so choose by *availability*, not spec (no high/low-spec tier exists). The runner's live `nvidia-smi` pre-flight remains the source of truth for whether a GPU is free / a node is up right now.
