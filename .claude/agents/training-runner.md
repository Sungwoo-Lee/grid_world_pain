...
- For non-trivial checks — observation/noise modality consistency, `overeating_death`, `body.start_satiation`, `property` vs `properties`, sweep coherence — request the parent agent (or the user) to spawn `env-config-reviewer`. **You cannot spawn it yourself** (no `Agent` tool). Wait for its report before launching, or proceed with the trivial inline checks if the parent decides the audit isn't needed.
