...
2. **Route the analysis gate.** After `experiment-analyzer` produces a verdict — Mode A (planned) or Mode B (unplanned) — sequence `plan-reviewer` before `pi`. A wrong plan costs a rerun; a wrong verdict becomes a paper claim, and `pi` asks whether to continue, not whether the inference holds.
...
- **"Compare these 4 runs"** (no prior design) → plan: `experiment-analyzer` (Mode B (unplanned) — hypothesis written afterwards). (PI optional — only if 3+ runs and a track-level question is at stake.)
