---
name: wiki-write
description: "Capture the current conversation as one or more session-insight files in this project's LLM Wiki at docs/llm_wiki/. Use whenever the user says 'remember this', 'save this', 'memorize this', '/wiki-write', 'save to memory', or session-end phrases like 'wrap up', 'wrapping up', 'we're done', 'good night', 'end of session', 'let's call it'. Also trigger when the user asks to capture a decision, finding, debugging conclusion, design rationale, rejected alternative, or 'what we learned' for future retrieval. The skill writes 16-field-frontmatter + 5-section insight files, updates the topic and global indexes under docs/llm_wiki/, and optionally archives a raw conversation. Use proactively at end-of-session even if the user does not name memory explicitly. Do NOT use for short typed rules another agent must obey on every invocation — those go to the built-in Claude Code auto-memory at ~/.claude/.../memory/MEMORY.md, which is a different layer with a different mechanism."
---

# Memorize — capture conversation insights into `docs/llm_wiki/`

This skill captures the current conversation into the LLM Wiki at `docs/llm_wiki/`. It produces one or more 5-section insight files, updates indexes, and optionally writes a raw-conversation archive.

**Authoritative contract**: `docs/llm_wiki/CLAUDE.md`. Treat it as ground truth for the frontmatter schema, naming rules, fragmentation safeguards, and raw-archive policy. This file describes the *invocation flow*; the operating manual carries the *contract*. If they disagree, the operating manual wins.

## When to use

Trigger on:

- `/wiki-write`, "remember this", "save this", "memorize this", "save to memory".
...
- Recall ("what did we decide?") → no skill needed; read `docs/llm_wiki/` directly.

## Quick reference (read first; details on demand)

| Thing | Value | Notes |
|---|---|---|
| Operating manual | `docs/llm_wiki/CLAUDE.md` | Read on every invocation. |
| Topic registry | `docs/llm_wiki/ROOT_INDEX.md` | Read before classifying (safeguard layer 1). |
| Tag dictionary | `docs/llm_wiki/entries/_global_tags.md` | Reuse existing tags. |
| Insight template | `docs/llm_wiki/TEMPLATES/entry.md` | Copy as starting point. |
| Code-graph snapshots | `docs/llm_wiki/code_snapshots/` | `python scripts/claude/snapshot_code_graph.py <label>` |
| Insight path | `docs/llm_wiki/entries/<topic>/<id>.md` | `<topic>` is English snake_case ≤ ~3 words. |
...
| Raw archive | `docs/llm_wiki/_archive/raw_conversations/<id>.md` | Gitignored (local-only). Pointed at by insight `raw_source`. |
...
1. `docs/llm_wiki/CLAUDE.md` — the contract.
2. `docs/llm_wiki/ROOT_INDEX.md` — every existing folder's 1-line definition.
3. `docs/llm_wiki/entries/_global_tags.md` — active tags.

If any is missing, halt: "`docs/llm_wiki/` is not seeded. Run the seed flow before invoking /wiki-write." Do not create the seed yourself.
...
Before writing a new insight, scan `status: settled` insights in folders sharing ≥ 1 tag. Per [CLAUDE.md §13](../../../docs/llm_wiki/CLAUDE.md#13-contradiction-handling-at-ingest):
...
2. Write to `docs/llm_wiki/entries/<topic>/<id>.md`. Start from `docs/llm_wiki/TEMPLATES/entry.md`. Fill all 16 frontmatter fields:
...
This is a hard step, not optional — the diary is the project's cross-session status board, and a wiki capture without a diary entry creates an invisible gap. If a diary call errors (e.g., `docs/diary/` is missing), surface the error to the user but do not roll back the insight writes; the insights are the durable artefact.

### Step 9 — Auto-commit the capture (mandatory)

After all writes succeed, bundle every file this skill touched into a single commit. The user has standing auto-commit authorization — do not ask. One coherent commit per `/wiki-write` run, regardless of how many insights or diary rows were produced.
...
```bash
/home/vncuser/miniconda3/envs/grid_world_pain/bin/python scripts/claude/regen_wiki_graph.py
```

Then stage by name only — include any insight files either regenerator touched (each prints which files it updated):

```bash
git add docs/llm_wiki/ROOT_INDEX.md \
        docs/llm_wiki/entries/_global_tags.md \
        docs/llm_wiki/entries/<topic>/_topic_index.md \
        docs/llm_wiki/entries/<topic>/<id>.md \
        docs/diary/<YYYY-MM-DD>.md \
        docs/llm_wiki/GRAPH_REPORT.md
# If regen_wiki_links.py updated older insights, add those paths too.
# If regen_wiki_graph.py updated Backlinks blocks in older insights, add those paths too.
# repeat the topic-index and insight paths for every (topic, id) pair written

git commit -m "$(cat <<'EOF'
docs(wiki): 📚 capture N insight(s) from <one-line session theme>
...
Match the existing project style: conventional-commit type `docs`, scope `wiki`, gitmoji `📚`, subject ≤ ~70 chars (check `git log --oneline -- docs/llm_wiki/` for tone). The body lists each insight as a bullet with topic/slug and the summary line — this is what makes the commit greppable months later.
...
- **Auto-commit the diary rows here, not in Step 8.** Step 8 calls `diary_append.py` without committing. This step bundles the diary file alongside the insights so the whole capture lands as one atomic commit. (Standalone `/diary` calls outside `/wiki-write` commit themselves — see `.claude/skills/diary/SKILL.md`.)

**Conflict recovery during worktree → user-branch propagation**

When `/wiki-write` runs from a worktree (background sessions) and propagates to the user's checked-out branch via `git -C <user-checkout> merge <worktree-branch> --ff-only`, the ff-merge may stall if the user has uncommitted edits in files the merge updates. Most common collision: `docs/llm_wiki/GRAPH_REPORT.md` (regenerated by multiple skills) and `docs/diary/<today>.md` (concurrent diary writes).

Recovery (deterministic, no LLM judgement needed — content is mechanical):

1. From the user checkout: `git stash push -u -m "wiki-write-propagation-overlap" -- docs/llm_wiki/GRAPH_REPORT.md docs/diary/<today>.md`
...
5. Re-run `python scripts/claude/regen_wiki_graph.py` to refresh GRAPH_REPORT.md with the merged state.
6. Commit the diary + GRAPH_REPORT updates as a small post-hoc fix-up: `docs(wiki): post-merge diary + GRAPH_REPORT refresh`.
...
> Saved N insight(s) to `docs/llm_wiki/` (and logged to today's diary):
>
> - `<topic>` — <one-liner>
> - `<topic>` — <one-liner>
>
...
- **Repo-relative paths only.** Never write to `~/.claude/...`, `/home/`, or any absolute path outside the current repo. The skill writes only inside the project tree under `docs/llm_wiki/` and `scripts/`. This matters in worktrees and sandboxes — absolute paths defeat isolation.
...
- "Delete" = move to `docs/llm_wiki/.trash/`. Hard-delete only on explicit user instruction.
- Fragmentation safeguards (4 layers) are non-optional; skipping any of them is a regression.
- If `docs/llm_wiki/CLAUDE.md` and this SKILL.md disagree, the operating manual wins — flag it and ask the user.

## References

- `docs/llm_wiki/CLAUDE.md` — operating manual (sections 4, 5, 6, 7, 11).
- `docs/llm_wiki/TEMPLATES/entry.md` — copy as starting point.
- `docs/llm_wiki/ROOT_INDEX.md` — topic registry.
- `docs/llm_wiki/entries/_global_tags.md` — tag dictionary.
- `scripts/claude/claude_jsonl_to_md.py` — full-raw archive helper.
- `docs/develop/active/meta/llm_wiki_system_design.md` — design rationale.
