---
name: wiki-read
description: "Surface insights from this project's LLM Wiki at docs/llm_wiki/. Use whenever the user asks 'what did we decide', 'what did we work on', 'remind me about', 'last week', 'yesterday', 'memory of', 'show me my memories', '/wiki-read', or any question about prior decisions, debugging conclusions, or design rationale that might live in the LLM Wiki. Two modes: natural-language recall (default — plain English, folder definitions hidden as definitions, time-aware) and technical query (raw indexes, folder names, counts) when the user says 'show ROOT_INDEX', 'list folders', 'audit', 'tag dictionary', 'fragmentation', 'L1/L2/L3'. Default behavior with no arguments: list the 10 most recent insights, reverse-chronological, then offer drill-down. Accepts optional range hints in the user phrasing ('today', 'yesterday', 'this week', 'earlier', 'all') for time-grouped output. Do NOT use this skill for capture — that's /wiki-write. Do NOT load raw archives (L4) without explicit user confirmation."
---

# Recall — surface insights from `docs/llm_wiki/`

This skill reads the LLM Wiki at `docs/llm_wiki/` and produces a human-friendly listing of stored insights, with optional drill-down to a specific insight's body. It is the read counterpart to `/wiki-write`.

**Authoritative contract**: `docs/llm_wiki/CLAUDE.md` §8 (natural-language recall) and §9 (lazy-load levels). Treat it as ground truth.

## When to use

Trigger on:

- `/wiki-read`, "show me my memories", "show recent memories".
...
- Capture ("remember this", "save this", "wrap up") — that's `/wiki-write`.
...
1. List matching snapshot files from `docs/llm_wiki/code_snapshots/` by reading `docs/llm_wiki/code_snapshots/README.md`.
...
1. Read `docs/llm_wiki/ROOT_INDEX.md` — gives the list of topic folders + their 1-line definitions + counts.
2. For each active topic folder listed, read `docs/llm_wiki/entries/<topic>/_topic_index.md` — gives all insights in that topic with date, time, id, summary.

If `docs/llm_wiki/` is missing or `ROOT_INDEX.md` shows 0 active folders, halt with:

> No wiki entries yet. Use `/wiki-write` to capture this conversation, or seed the `docs/llm_wiki/` layer first.
...
Use the folder's **definition** from `ROOT_INDEX.md` (e.g. *"Claude LLM Wiki's own design decisions"*), NOT the raw folder name (`wiki_system_design`). Italics signal "this is the topic, in plain English".
...
```
ROOT_INDEX.md (docs/llm_wiki/ROOT_INDEX.md)
...
Per-topic indexes:
- entries/<folder>/_topic_index.md: N insights, last YYYY-MM-DD
...
```
...
- **Repo-relative paths only.** Read only inside `docs/llm_wiki/`. Never read `~/.claude/projects/.../memory/MEMORY.md` from this skill (different layer; recall there is the harness's job).
...
- **Do not capture** anything from this skill. If the listing reveals something the user wants to save, route them to `/wiki-write`.
- If `docs/llm_wiki/CLAUDE.md` and this SKILL.md disagree, the operating manual wins — flag and ask.

## References

- `docs/llm_wiki/CLAUDE.md` §8 (natural-language recall format), §9 (lazy-load levels), §10 (large-file warning protocol).
- `docs/llm_wiki/ROOT_INDEX.md` — topic registry + folder definitions.
- `docs/llm_wiki/entries/<folder>/_topic_index.md` — per-topic insight rows.
- Companion skill: `.claude/skills/wiki-write/SKILL.md` (capture; this skill is recall).
