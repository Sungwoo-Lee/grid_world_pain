...
- **Filename pattern**: `YYYYMMDD_HHMM_<slug>.md` (recommended) over `YYYY-MM-DD_<slug>.md` / range-encoded / slug-first. Matches `.claude-memory/` insight convention; sortable; minute-granular.
...
- **Scope discovery**: hybrid — user names the study; skill auto-discovers experiments from `docs/experiments/active/<topic>/` + `.claude-memory/memories/<topic>/`; always confirms the auto-detected list before writing.
