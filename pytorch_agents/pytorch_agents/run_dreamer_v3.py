"""Thin sheeprl entry point: DreamerV3 + grid_world_pain metric parity.

# GWP-PATCH-FILE-VERSION: sheeprl 0.5.8.dev
# Vendored from:
#   sheeprl/algos/dreamer_v3/dreamer_v3.py  (main() body, lines ~360–780)
...
The ``train()`` function and all helpers are imported unchanged from sheeprl.
Only the ``main()`` body (the env-interaction loop) is vendored here, with
the two GWP-PATCH blocks added.

Usage (via launch_sheeprl.sh, not intended for direct invocation):
    export GWP_CONFIG_PATH=<path>
    export SHEEPRL_SEARCH_PATH=pkg://pytorch_agents.configs
    python -m pytorch_agents.run_dreamer_v3 \\
...
# GWP: import our metric-parity helpers
from pytorch_agents.aggregators.behavior_measures import (
    register_dynamic_keys,
    update_from_final_info,
)

# Import sheeprl's train() function verbatim (the model-update step is unchanged)
from sheeprl.algos.dreamer_v3.dreamer_v3 import train


def main(fabric: Fabric, cfg: Dict[str, Any]):
    """Vendored DreamerV3 main loop with grid_world_pain metric parity patches.

    Identical to sheeprl.algos.dreamer_v3.dreamer_v3.main() except for:
      - GWP-PATCH-A: register per-tag Episode/* keys on the MetricAggregator
        immediately after it is instantiated.
      - GWP-PATCH-B: call update_from_final_info(aggregator, agent_ep_info)
        inside the "final_info in infos" block so terminal-info keys our
        wrapper emits reach WandB via sheeprl's existing fabric.log_dict path.
    """
    ...
    # Create Logger. This will create the logger only on the rank-0 process
    ...
    # -----------------------------------------------------------------------
    # GWP-PATCH-A: register per-tag Episode/* keys on the MetricAggregator.
    # neutral_tags and predator_tags come from the first wrapped env's params.
    # envs.envs[0] is the RestartOnException wrapper; unwrap until we reach
    # our GridWorldPainWrapper (which has _neutral_tags / _predator_tags).
    # -----------------------------------------------------------------------
    try:
        inner_env = envs.envs[0]
        while hasattr(inner_env, "env"):
            inner_env = inner_env.env
        neutral_tags  = inner_env._neutral_tags
        predator_tags = inner_env._predator_tags
        register_dynamic_keys(aggregator, neutral_tags, predator_tags, device=str(device))
        fabric.print(
            f"GWP-PATCH-A: registered {len(neutral_tags)} neutral + "
            f"{len(predator_tags)} predator tags on aggregator"
        )
    except Exception as _e:
        fabric.print(f"GWP-PATCH-A: could not register dynamic keys — {_e!r}")
    # -----------------------------------------------------------------------
            ...
            if cfg.metric.log_level > 0 and "final_info" in infos:
                for i, agent_ep_info in enumerate(infos["final_info"]):
                    if agent_ep_info is not None:
                        ...
                        # ---------------------------------------------------
                        # GWP-PATCH-B: forward our Episode/* terminal keys
                        # to sheeprl's aggregator so they reach WandB via the
                        # existing fabric.log_dict path (see log metrics block
                        # below, at cfg.metric.log_every cadence).
                        # ---------------------------------------------------
...
# Hydra entry point
#
# We replicate sheeprl.cli.run() behaviour here, but dispatch to our own
# patched ``main`` function above rather than sheeprl's original.
# sheeprl.cli.run_algorithm() sets up Fabric, the reproducibility wrapper,
...
def cli(cfg: DictConfig) -> None:
    ...
    Equivalent to ``python -m sheeprl exp=dreamer_v3_grid_world_pain ...``
    but with GWP-PATCH-A and GWP-PATCH-B applied to the DreamerV3 main loop.
    """
    import importlib
    import types
    ...
    # Trigger registration of sheeprl's dreamer_v3 so the module entry exists
    # (importing the module calls @register_algorithm())
    importlib.import_module("sheeprl.algos.dreamer_v3.dreamer_v3")

    # Inject our patched main into sys.modules so run_algorithm can import it
    synthetic_mod = types.ModuleType("sheeprl.algos.dreamer_v3.dreamer_v3_gwp")
    synthetic_mod.main = main  # our patched version above
    sys.modules["sheeprl.algos.dreamer_v3.dreamer_v3_gwp"] = synthetic_mod

    # Add a registry entry for "dreamer_v3_gwp" under the dreamer_v3 module
    module_key = "sheeprl.algos.dreamer_v3"
    gwp_entry = {"name": "dreamer_v3_gwp", "entrypoint": "main", "decoupled": False}
    if module_key not in algorithm_registry:
        algorithm_registry[module_key] = [gwp_entry]
    else:
        # Only add if not already present (idempotent for restarts)
        if not any(e["name"] == "dreamer_v3_gwp" for e in algorithm_registry[module_key]):
            algorithm_registry[module_key].append(gwp_entry)

    # Now run using sheeprl's full CLI plumbing (Fabric setup, repr wrapper,
    # MetricAggregator.disabled logic, etc.) but pointing at our algorithm.
    print_config(cfg)
    if cfg.checkpoint.resume_from:
    ...
    # Override algo.name so run_algorithm dispatches to our patched main
    cfg_dict.algo.name = "dreamer_v3_gwp"
    check_configs(cfg_dict)
    run_algorithm(cfg_dict)
