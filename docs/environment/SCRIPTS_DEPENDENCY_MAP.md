...
*Verbatim reference strings and line numbers above are the authoritative "what to rewrite" list. Last updated: 2026-07-22 (added `scripts/eval/dwell_sweep/` — the committed dwell-history sweep pipeline promoted from this session's `tmp/` scratch scripts). Previously updated 2026-06-30 after the scripts/ folder reorganization (Phases 1–6). Re-run the sweep above if `scripts/` callers change substantially.*
