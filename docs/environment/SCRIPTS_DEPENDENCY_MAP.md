...
| `scripts/eval/dreamer_srl_probe_eval.py` | none yet (hand-run driver, added 2026-07-14, for running dreamer_srl checkpoints through behavior-probe env configs; `eval_rollout.py` raises `NotImplementedError` for Dreamer) | HAND | depth fix only (`scripts/eval/`, already `parents[2]`) |
