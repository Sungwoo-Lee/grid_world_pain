...
| `scripts/eval/probe_eval_checkpoint.py` | `sys.path.insert(0, PROJECT_ROOT / "scripts" / "eval" / "dwell_sweep")` then `from run_sweep import _measure_cell, HEAD, KEYS` (bare-name import, added 2026-07-24 — the during-training behavior-probe eval core reuses `run_sweep.py`'s measure/aggregate logic verbatim rather than reimplementing it; see docs/develop/active/behavior/PROBE_EVAL_DURING_TRAINING.md) | `scripts/eval/dwell_sweep/run_sweep.py` (which itself re-exports `avoidance_stats_heatmap.KEYS` as `HEAD = ["step","step_M"] + KEYS`) |
...
| `train.py` (`_PROBE_EVAL_SCRIPT` constant, `_maybe_dispatch_probe_eval()`) | `_PROBE_EVAL_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts", "eval", "probe_eval_checkpoint.py")` → `subprocess.Popen([sys.executable, _PROBE_EVAL_SCRIPT, --checkpoint, --result-json, --out-root, --conditions, --episodes, --checkpoint-key, --global-step, --iteration], env=...)`, dispatched async (non-blocking, never `wait()`-ed in the main loop) at every Nth checkpoint save when `training.probe_eval_during_training` is true (added 2026-07-24 — see docs/develop/active/behavior/PROBE_EVAL_DURING_TRAINING.md) | `scripts/eval/probe_eval_checkpoint.py` |
...
**Rule:** moving `render_recordings.py` requires editing both lines above. Moving `probe_eval_checkpoint.py` requires updating `train.py`'s `_PROBE_EVAL_SCRIPT` constant. (`evaluation_core.py:328` also prints a cosmetic "Render with: python scripts/eval/render_recordings.py ..." hint — non-breaking, but update for tidiness.) Moving `eval_rollout.py` requires updating the harness's `EVAL_ROLLOUT` constant (it lives in the same `scripts/eval/` folder today, so this is currently a same-folder reference, not a cross-folder one).
