...
| `sensory.olfactory_grid_range` | **0** | `default.yaml` | Radius of the olfactory sampling diamond. `0` = pre-DIRECTIONAL_SENSORS single sample (byte-identical). `>0` adds `(2r²+2r+1)×vector_size` dims — **changes `obs_dim`, breaking checkpoint compatibility**. |
