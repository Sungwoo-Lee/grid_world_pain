...
status: active
created: 2026-05-13
last_updated: 2026-05-14  # CP3 → CP-PASS — zero-init reward + critic heads (cascade fix #27); 4-gate closure verified (zero is platform-invariant)
...
6. **CP4 + CP4b** → tests → gate → CP-PASS. **← NEXT** (single developer task
   covers both since they share the same file: CP4 extends `agent.py` with the
   `RSSM` nnx.Module — transition + representation models each with one hidden
   MLP layer, cascade fix #30 from the v2 plan — and `get_initial_states`
   returning the transition's *mode* without consuming PRNG; CP4b adds the
   §S4 three-quantity arithmetic-mask `is_first` reset on the scan output
   (action, recurrent_state, posterior with `[B, S, D] → [B, S*D]` reshape-flatten
   BEFORE masking), in arithmetic form `(1 - is_first) * x + is_first * init`,
   NOT `jnp.where`. Sheeprl references: `vendor/sheeprl/sheeprl/algos/dreamer_v3/agent.py:L1021-L1051`
   (RSSM) and `:L423-L429` (§S4 reset). Per CP3b professor hand-off:
   instantiate `LayerNormGRUCell` (CP2) with the existing `eps=1e-3` default;
   apply Hafner `init_weights` at the RSSM construction site, NOT inside the
   cell; wrap as a `jax.lax.scan` step closure; do NOT modify
   `LayerNormGRUCell.__call__`. The §S4 arithmetic-mask lives OUTSIDE the cell
   on the scan output — do NOT add an `is_first` parameter to the cell's
   `__call__` signature. ~3–4 days estimate; CP4 is one of the heavier CPs).
