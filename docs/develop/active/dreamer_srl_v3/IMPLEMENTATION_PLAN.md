...
status: active
created: 2026-05-13
last_updated: 2026-05-14  # CP6 → CP-PASS — 4-gate closure verified (process discipline restored); 32/32 Lever-A PASS at the PI-raised D-010 5e-5 threshold; reviewer chain ✅ (5458c0c); PI ratified D-010 at fa84099 (threshold raised 4e-5 → 5e-5 for margin-band consistency); CP7 next
...
8. **CP7** → tests → gate → CP-PASS. **← NEXT** (developer task extends the
   existing `src/algorithms/dreamer_srl/train.py` from CP6 with: (a) the
   Polyak target-critic EMA update `target_param = (1 - tau) * target_param +
   tau * online_param`, with first-call init copying online → target (hard
   copy) and subsequent calls blending; (b) the actor REINFORCE objective
   `log_prob(sg(action)) * sg(advantage)` where `advantage = (lambda_value -
   baseline) / max(1, high - low)` per CP1's `Moments`. Lever-A tests per
   `CHECKPOINT_REGISTRY`: `test_polyak_first_call_hard_copy` (tau=1 path),
   `test_polyak_subsequent_call_blend` (tau=0.02 path), `test_polyak_fires_before_train_step`
   (call-order in the gradient loop — Polyak fires BEFORE `one_train_step`, not
   after, per the CHECKPOINT_REGISTRY pre-CP4 hand-off note). The CP4
   professor's CP7 hand-off note ([docs/reviews/dreamer_srl_v3_cp4_professor_rl_bayesian_dl_review.md](../../../reviews/dreamer_srl_v3_cp4_professor_rl_bayesian_dl_review.md))
   ties the actor advantage normalisation to CP1's `Moments` (the §S5
   true-continue splice at imagination step 0 — pre-CP4's continue head output
   spliced with observed `1 - terminated[0]` at imagination index 0 — and the
   §S7 advantage low-offset cancellation `(λ - offset) - (baseline - offset)`
   where the offset cancels algebraically). Sheeprl references for the
   developer prompt: `vendor/sheeprl/sheeprl/algos/dreamer_v3/dreamer_v3.py:L246-L294`
   (actor body) and `:L325-L335` (Polyak update at end of train step).
   ~1–2 days estimate per the v3 cost-projection table.)
