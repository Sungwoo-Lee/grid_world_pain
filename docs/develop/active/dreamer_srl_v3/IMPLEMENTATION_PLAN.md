...
status: active
created: 2026-05-13
last_updated: 2026-05-14  # CP10b → ⚠ VERIFICATION HALTED. 20,000-step dreamer-srl smoke on corrected XS config (`configs/dreamer_srl/01_food_only.yaml` post-fix: 256/1/256/256/24, learning_starts=1024, single RTX 6000 Ada on node 114 GPU 0) — WandB run [s31wc1a1](https://wandb.ai/sungwoolee/grid_world_pain_dreamer_srl_smoke/runs/s31wc1a1), 1252.4s wall-clock, **16.0 env-SPS final**. **5 of 6 acceptance gates close cleanly**: Gate 1 JIT compile + iter loop progresses to step 20000 ✅ (single-GPU comfortably handles corrected XS, post-completion GPU 0 = 1 MiB used — **headline empirical closure of D-013 substrate concern**); Gate 2 no NaN ✅ (0 hits across 7 Loss/* keys); Gate 4 episode-len logging ✅ (7 distinct ep_len values across 198 episodes, bulk 101 = timeout cap, same pattern as CP10); Gate 5 moments_invscale ≥ 1.0 ✅ (min 4.82, max 61.10, final 6.42 — never below safety floor); Gate 6 parity-launch projection ≤ 25h ✅ (200,000/16.0 = **3.47h/seed**, 7.2× inside the 25h budget gate, dreamer-srl is **3.60× faster than sheeprl's 12.5h baseline** on the same recipe). **Gate 3 (WM-loss drop ≥ 20%) ❌ FAIL**: drop = 10.21% (first-log iter 1024 → final-log iter 20000) / 7.55% (first-5 mean → last-5 mean) / 4.15% (first-5k mean → last-5k mean, CP10 methodology). All three methodologies fall below the ≥ 20% threshold AND below the 15-20% PASS-with-note band the senior-developer agent profile carries. **Structural explanation**: the gate's ≥ 20% rule was inherited from CP9 / CP10 (both `learning_starts: 0`); the parity-track value `learning_starts: 1024` means no WM-loss is logged before iter 1024, and at iter 1024 the D-014 debt-repayment burst has applied 1024 grad steps in one go so the "first" log (1.5641) is itself a post-1024-grad-steps value, not a pre-training baseline. The final WM-loss 1.4044 is essentially identical to CP10's final 1.410 — the model reaches the same floor, it just starts much closer to it because the corrected XS model is much smaller. This is the first run where the inherited gate methodology does not apply; the failure is a methodology-rescoping question, not a training-health issue. Senior-developer halts the verification per "if any gate fails, halt the flip" rule (autonomous-run cannot relax a gate below PASS-with-note band). Three dispositions surfaced for user / PI: (α) accept strict fail / CP10b BLOCKED; (β) re-scope Gate 3 to "final WM-loss within ±10% of CP10's final" (CP10b lands at −0.4% delta, would PASS); (γ) run complementary `learning_starts: 0` smoke as the WM-loss-descent check. Senior-developer reading: β is cleanest on the merits but methodology re-scoping is user / PI scope. CP10b verification-table row stays NOT FLIPPED at this HEAD. The headline closures (D-013 substrate concern empirically dismissed at corrected XS; parity-launch budget trivially met at 3.47h/seed) are nonetheless fully established by the other 5 gates and are not in dispute.
...
### CP10b — post-D-013-correction wall-clock at corrected sheeprl XS (single GPU)

Status: **⚠ VERIFICATION HALTED (2026-05-14)** — 5 of 6 acceptance gates close cleanly, but the **WM-loss-drop gate (Gate 3) fails strictly on every methodology tried (10.2% / 7.6% / 4.2% vs the ≥ 20% threshold)**, and the failure is below the 15–20% "PASS-with-note" band the senior-developer agent profile carries for this gate. The verification-table row is **NOT flipped to CP-PASS at this HEAD**; user / PI disposition is needed on whether the failed gate reflects a real training-health issue or a structural artifact of the new `learning_starts: 1024` baseline-window assumption that the gate inherited from CP9 / CP10 (both of which ran `learning_starts: 0`).

##### Plain-language CP10b verification-halt note

**The plain-English headline.** A 20,000-environment-step smoke of the JAX dreamer-srl rebuild ran on the corrected food-only configuration — the one whose dense-layer width, MLP depth, recurrent-state size, and CNN channel multiplier were just edited to mirror the real sheeprl-XS preset (256 units / 1 layer / 256 recurrent / 24 CNN multiplier) rather than the XL-equivalent values they had been mis-carrying. The run completed without crashing, used a single RTX 6000 Ada GPU on lab node 114, did not run out of memory, did not produce any NaN values in any loss, and finished at **16.0 environment-steps-per-second**. Projecting that throughput to the parity launch's 200,000-step target gives **~3.5 hours per seed**, well inside the ≤ 25-hour budget (the budget is twice sheeprl's reference 12.5-hour run on the same recipe). Five of the six acceptance gates close cleanly. **One gate fails**: the world-model loss dropped only ~10% across the run, below the ≥ 20% threshold inherited from CP9 / CP10. The failure has a clean structural explanation, but it is below the PASS-with-note band the agent profile carries for this gate, so the verification halts here for user / PI disposition rather than a unilateral senior-developer relax. The big empirical headline — *the corrected XS config compiles and trains on a single GPU, the 14.38 GB OOM that D-013 surfaced does NOT recur, and the parity-launch wall-clock budget is comfortably met* — is fully closed. The blocking question is narrowly methodological: whether the WM-loss-drop gate written for `learning_starts: 0` is the right gate to apply when `learning_starts: 1024` makes the early-window baseline structurally different.
...
| Gate 3 — WM-loss drop ≥ 20% over the run | ❌ **FAIL** | First post-prefill log at iter 1024: `world_model_loss=1.5641`. Final log at iter 20000: `world_model_loss=1.4044`. Drop = `(1.5641 − 1.4044) / 1.5641 = 10.21%`. Method-2 (first-5 mean vs last-5 mean): 1.5346 → 1.4187 = **7.55%**. Method-3 (first-5k-step mean vs last-5k-step mean, the CP10 methodology): 1.4868 → 1.4251 = **4.15%**. All three methodologies fall below the ≥ 20% threshold AND below the 15–20% "PASS-with-note" band the senior-developer agent profile carries for this gate. **Structural explanation**: (a) the corrected XS model is much smaller than CP10's reduced-dim model (256 dense × 1 mlp layer × 256 recurrent vs CP10's 256 dense × 8×8 stochastic × untrue 1024 recurrent shape), so its initial WM-loss landed already at 1.564 instead of CP10's 2.136 — there is less headroom to drop into; (b) `learning_starts: 1024` (parity-track) means no WM-loss is logged before iter 1024 — and at iter 1024 the D-014 debt-repayment burst has already applied 1024 gradient steps in one go, so the "first" log is itself a post-1024-grad-steps value, not a pre-training value; (c) the **final WM-loss 1.4044 is essentially identical to CP10's final 1.410** — the model reaches the same floor, it just starts much closer to it. The gate's ≥ 20% rule was inherited from CP9 / CP10 (both `learning_starts: 0`) without revisiting whether it applies under the parity-track `learning_starts: 1024` value; this is the first run where it does not. |
...
##### Disposition request — user / PI

**Five of six gates close cleanly; one fails strictly.** The failing gate (Gate 3, WM-loss drop ≥ 20%) has a clean structural explanation rooted in the parity-track `learning_starts: 1024` value invalidating the gate's inherited-from-CP9/CP10 step-200 baseline assumption — but the failure (10.2% / 7.6% / 4.2% on three methodologies) sits below the 15–20% PASS-with-note band the senior-developer agent profile carries for this gate. The senior-developer judgment under autonomous-run constraint cannot relax a gate below the PASS-with-note band; this is squarely user / PI scope. Three plausible dispositions:

1. **Disposition CP10b-α — accept the strict fail, declare CP10b BLOCKED.** The ≥ 20% rule is the rule; CP10b cannot close CP-PASS with a 10.2% drop. CP10b stays at ⚠ VERIFICATION HALTED, the parity launch is gated on resolving the WM-loss-descent question (e.g., by running a longer smoke with `learning_starts: 0` to confirm the corrected XS model can in fact drop WM-loss ≥ 20% when given a fair pre-training baseline). This is the conservative reading.

2. **Disposition CP10b-β — re-scope the WM-loss-drop gate to `learning_starts > 0` runs.** Acknowledge that the ≥ 20% rule was a CP9/CP10 artifact tied to `learning_starts: 0`; re-state CP10b's WM-loss-drop gate as *"final WM-loss is within ±10% of CP10's final WM-loss 1.410"* (CP10b's 1.4044 lands at −0.4% delta, well inside ±10%). Under this re-scoped gate, CP10b closes CP-PASS. The senior-developer would not make this re-scoping call autonomously — it is a methodology change to a published acceptance criterion, and methodology changes are user / PI scope.

3. **Disposition CP10b-γ — run an additional learning_starts=0 smoke as the WM-loss-descent check.** Hold CP10b open as ⚠ VERIFICATION HALTED while a complementary 20,000-step run with `learning_starts: 0` (otherwise identical config) is launched; if that run hits the ≥ 20% drop, CP10b CP-PASSes on the combined evidence. This is the "show your work" disposition — it produces a clean ≥ 20% WM-loss-drop measurement at the corrected XS config without re-scoping any rule.

**The senior-developer's reading**: Disposition CP10b-β is the cleanest and is correct on the merits — the ≥ 20% rule was written for a `learning_starts: 0` baseline, the parity-track is `learning_starts: 1024`, the corrected XS model reaches the same WM-loss floor as CP10, and the original Gate-3's intent (catch a stuck / diverging model) is fully satisfied by the WM-loss landing at 1.4044 (the same floor as CP10's 1.410). But this is methodology re-scoping and so is user / PI scope, not senior-developer scope under autonomous-run constraint. **The verification halts here; the CP10b verification-table row remains `NOT FLIPPED` at this HEAD**, and the diary records `⚠ VERIFICATION HALTED — Gate 3 fail, user/PI disposition needed`. The big empirical closures (D-013 substrate-class concern empirically dismissed, parity-launch wall-clock budget trivially met at 3.47 h/seed) are nonetheless fully established by the other 5 gates and are not in dispute.

##### Note for parity-launch planning

The Gate-3 failure does **NOT** indicate a training-health issue. The five other gates all close cleanly:
- JIT compile fits the corrected XS on a single GPU with massive headroom (D-013 substrate concern dismissed).
- No NaN, no divergence, no safety-floor activation.
- 7 distinct ep_len values logged across 198 episodes.
- moments_invscale stayed ≥ 4.82 throughout (4.8× above the safety floor of 1.0).
- The parity-launch projection is 3.47 h/seed, 7.2× inside the 25 h budget gate.

The big-picture closures CP10b was scoped to establish — *D-013 is a config-correctable mis-port, not a substrate constraint, and corrected XS trivially fits the parity-launch budget* — are both fully established by the run, independent of how Gate 3 is resolved. The parity launch's go / no-go is therefore not blocked by this halt; it remains blocked by the parity-launch PI consultation choosing a (size, num_envs) configuration (informed in part by the SPS sweep running on GPUs 1/2/3 of the same node in parallel — separate downstream analysis).
