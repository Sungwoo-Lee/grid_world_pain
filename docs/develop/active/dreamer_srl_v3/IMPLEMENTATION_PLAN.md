...
7. **CP6** → tests → gate → CP-PASS. **← NEXT** (developer task targets a NEW
   file `src/algorithms/dreamer_srl/train.py`: critic loss with EMA
   self-regularisation per Hafner §3.3 — cascade fix #29 from the v2 plan.
   Two `log_prob` terms: `-qv.log_prob(stop_gradient(lambda_target))` AND
   `-qv.log_prob(stop_gradient(target_critic_value))` — both required, the
   target_critic_value term is the slow-target regulariser. Plus §S6
   discount weighting `cumprod(continues * gamma) / gamma` applied to both
   terms. Plus §S8 free-nats per-element floor `max(KL, free_nats)` applied
   BEFORE the mean. Plus §S9 `Independent(Bernoulli, 1)` wrap on the continue
   head. Lever-A tests per `CHECKPOINT_REGISTRY`: `test_critic_loss_two_terms`,
   `test_critic_target_lambda`, `test_discount_weighting`. Sheeprl reference:
   `vendor/sheeprl/sheeprl/algos/dreamer_v3/dreamer_v3.py:L240-L320`
   (`train()` body, in particular the critic-loss assembly). Per CP3 + CP5
   professor hand-off notes: the zero-init reward + critic heads landed at CP3
   bootstrap the lambda-target signal cleanly from zero at iter-0; CP5's
   `TwoHotEncoding.log_prob` already symlog-encodes targets internally, so the
   developer passes raw scalar `lambda_target` / `target_critic_value` to it.
