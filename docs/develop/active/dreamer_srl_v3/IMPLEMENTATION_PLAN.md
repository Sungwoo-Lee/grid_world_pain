...
status: active
created: 2026-05-13
last_updated: 2026-05-14  # CP9 → CP-PASS — integration-smoke close on the 5,000-step WandB dry-run (ki4qwwk0): 36/36 pre-flight Lever-A pytest PASS in 52.15s (no regression), 3-of-3 offline_check.py runs PASS at 17/17 with max drift 4.768e-7 (no flap), all three sanity conditions PASS (no NaN; world-model loss drop 30.1% ≥ 20%; Game/ep_len_avg logged at 49 episode boundaries with 3 distinct values), CP8 hand-off guard Diagnostic/moments_invscale ≥ 1.0 holds (min 1.0000, max 6.5517). Two deviations: D-012 (learning_starts=0 for CP9 smoke) ✅ APPROVED by senior-developer at CP9 verification per pre-declared plan-time disposition (CP9 is reviewer-optional per checkpoint table line 526, no PI gate fires for CP9); D-013 (full XS config OOMs on single RTX 4090 during JIT compile of one_train_step at 14.38 GB, smoke ran reduced-dim config 256 units / 8×8 stochastic / horizon=7) ☐ pending — substrate-class match with D-004 memmap omission, disposition deferred to parity-launch PI consultation per pi.md portfolio-level config trigger. Speed check N/A (first runtime path; CP10 measures parity-launch wall-clock budget). CP9b (random-action prefill §S3) is next eligible
