...
> **Status**: PLANNED, rev 5 (round-3 review verdict NOT READY resolved — Gate 1c vacuous-pass fixed, curriculum-resume case added, live-run landing decision recorded; one decision-delta still flagged for user confirmation at approval — see "Binding-decision status" below)
> **Opened**: 2026-07-24
...
- rev 5 (2026-08-03): round-3 plan-review corrections (`docs/reviews/plan_dreamer_integration_rev4.md`, verdict NOT READY; feedback appended below) — **F1 (🔴)**: Gate 1c's episode budget stated as ABSOLUTE (`while total_episodes_completed < episodes`), invocation pinned to `--load-episode 20 --episodes 40`, and a **non-vacuity pass criterion** added (non-empty telemetry + ≥1 post-`train_start_iter` loss-updating iteration — also makes an Adam-restore divergence observable through post-resume parameter updates); **F2 (🟡)**: Gate 1c gains a second case, *1c-curriculum*, resuming into the 1b fixture's second stage so §12c stage-rebuild-on-resume is exercised through train.py's schedule+resume plumbing in combination; **F3 (🟡)**: live-run landing decision recorded as a reversible default — **defer merging the `dreamer_srl_main.py` refactor to `v3.0` until the live run completes** (development + Gate 1 proceed on a feature branch; option (b) crash-relaunch-on-refactored-code documented as the user-selectable alternative); **F4 (🟢)**: stale anchors fixed (R7 wandb labels :944/:980; checklist budget-deletion keep-line :593); **F5 (🟢)**: commit attributions corrected — resume flags/restore = `166f261` (`e834ec1` = §12c stage rebuild + RollingWindow only), `testing.seed` switch = `b228117`; `10000af` (async checkpoint-video render) and `0030c04` (hierarchical encoder) also touched the file in the window, verified no CLI-flag or below-seam `args.*` impact.
...
- **Landing decision for the Phase 1 refactor (rev 5, round-3 finding 3 — recorded as a reversible default; user can veto at approval).** Chosen: **(a) defer the merge**. Phase 1 rewrites `main()` in the exact file a crash-relaunch (with `--load-checkpoint`, through §12b/§12c) would execute, and no gate compares refactored-legacy against **pre-refactor** legacy — Gate 1's A-side is already the refactored legacy, so equivalence at relaunch would rest solely on Checkpoint 1 (test suite + smoke) and the byte-diff review. Deferring costs only calendar time: development and all Gate 1 legs proceed on a **feature branch/worktree**, and the merge to `v3.0` waits for the live run to complete — which also aligns the code landing with the already-deferred config commits (one consolidated landing event, one thing for the user to clear). Documented fallback **(b)**, selectable by the user if speed matters more: land Phase 1 on `v3.0` immediately, accept Checkpoint 1 as the sole crash-relaunch guard, and record a pinned relaunch procedure — a `git worktree` checked out at the pre-refactor commit (the hash recorded in this doc at landing time), from which any crash-relaunch of the live run is executed byte-identically to the original launch.
...
8. **Rev-4 stack reconciliation** (four commits landed 2026-07-28/29; baseline now dreamer_srl_main.py 2,425 lines / train.py 2,523 lines, train.py anchors +1: peek :508, stage-0 alias :564, propagation loop :556-561, algorithm read :650, probe gate :669, signal registration :449-450):
   - `166f261` — **dreamer resume exists** (see Binding-decision status): flags at dreamer_srl_main.py:497-506, restore block §12b at :1369+ (inside the extracted body → two new spec fields). (Rev-4 misattributed this to `e834ec1`; corrected per round-3 finding 5.)
   - `e834ec1` — the curriculum **stage-rebuild-on-resume fix** (§12c, :1437+) and the `RollingWindow` partial-window emission + `Episode/_window_n` — the latter a shared `src/utils/rolling_logging.py` change hitting both entry points identically, so parity is unaffected.
   - `b228117` — checkpoint-eval seeding switched to the **`testing.seed`** evaluation key (`eval_seed`, :712) — the eval passes no longer consume `args.seed`. (Rev-4 misattributed this to `ad8929a`'s region; corrected.)
   - `ad8929a` — startup **retention guard** (warn-only) for curriculum cadence × `training.max_checkpoints_to_keep`; sits inside the extracted body, uses existing keys, no spec impact.
   - `10000af` (async checkpoint-video render) and `0030c04` (hierarchical encoder) also touched dreamer_srl_main.py in this window — **verified by the round-3 reviewer: neither adds CLI flags nor new below-seam `args.*` reads**, so no spec impact.
...
**How captured — Gate 1c (resume leg — rev 4, forced by the resume commits `166f261`/`e834ec1`; spec hardened in rev 5 per round-3 findings 1–2)**: two cases, both through both entry points with `--parity-dump`, same seed.

*Budget semantics (the round-3 vacuous-pass trap, stated explicitly)*: the loop condition is `while total_episodes_completed < episodes` (dreamer_srl_main.py:1606) and §12b **restores** `total_episodes_completed` from the checkpoint — `--episodes` is an **absolute total, not an increment**. Resuming at episode 20 with `--episodes 20` makes the condition false on entry: the run exits instantly, both sides produce identical *empty* telemetry, and every equality criterion passes while proving nothing. The invocations below are pinned so the resumed segment must actually train, and pass criterion 4 makes an empty resume a hard FAIL.

- **1c-single** (1a fixture): complete one 1a run (40 episodes, checkpoints at 20/40), then resume both entry points from the same checkpoint: `--load-checkpoint <1a-run>/checkpoints --load-episode 20 --episodes 40` → a real 20-episode resumed segment. Exercises §12b restore (networks + counters + moments + Adam state) and the `train_start_iter` buffer-refill gate — which only fires if the loop runs.
- **1c-curriculum** (1b fixture — round-3 finding 2): reuse the completed 1b run dir and resume **into the second stage**: `--configs-dir <1b-fixtures> --continual-schedule <1b-sched> --load-checkpoint <1b-run>/checkpoints --load-episode 40`. No `--episodes` flag — curriculum mode forbids budget flags and takes the budget from `episode_boundaries[-1] = 60`, giving a 20-episode resumed segment inside stage index 1 (past the stage boundary at 30). This is the only place §12c stage-rebuild-on-resume (`if args.load_checkpoint and schedule is not None`, :1437) fires **through train.py's schedule construction + resume-flag plumbing in combination** — the retargeted `test_continual_resume_rebuild.py` drives the seam directly and cannot cover that interaction.
...
2. *Numeric parity, CPU (`JAX_PLATFORMS=cpu`)*: **bit-identical** — `np.array_equal` on every telemetry array, exact equality on all integer counters, PRNG key, and checksums; in 1b this includes the swap-iteration index and post-swap counters; in 1c the restored counters (`policy_step`, episodes, grad-steps), the post-refill buffer fill, the final checksums after the resumed segment, and (1c-curriculum) the resumed stage index. Bit-identity is the target and is achievable because both processes execute the *same* `run_dreamer_training` function with identical inputs on a deterministic backend.
...
4. *Non-vacuity (rev 5, round-3 finding 1 — applies to every leg, binding for both 1c cases)*: the telemetry arrays are **non-empty**, and at least one iteration **after `train_start_iter`** shows `last_losses` updated (i.e., a gradient-taking iteration ran in the compared segment). An instant-exit run — however perfectly it matches the other side's instant exit — is a **FAIL**. Side effect that makes this criterion load-bearing beyond vacuity: Adam optimizer state is never directly checksummed in the dump, so a missing/incorrect Adam restore is observable *only* through post-resume parameter updates — which requires resumed training to happen.
...
| R7 | `agent.algorithm` rename breaks WandB dashboard filters and any tooling matching `DreamerV3` | new runs invisible in old views — **and without the seam label edits (File Change 1, lines :944/:980) the wandb.config would keep saying `DreamerV3` while the dumped configs say `dreamer_srl`, a worse mixed-identity state** | spec-driven label (finding 2 fix); enumerated (compat C7); Gate 2 analyst confirms new runs filterable; grep for `DreamerV3` consumers in `scripts/`/`analysis/` during Phase 1 |
...
| R12 | (rev 4) Integration work disturbs the **live** 3-stage curriculum run on the legacy entry point, or resume-path plumbing through train.py silently diverges from the legacy resume | a crash-relaunch of the live run fails to reproduce its invocation; or a train.py-resumed run restores differently than a legacy-resumed one | Live-run constraint section (config-edit freeze incl. the `fb54bc0` curriculum configs, **Phase-1 merge deferred per the recorded landing decision (a)**, Gate 3 deferred, Gate 2 node avoidance, results-dir isolation); **Gate 1c both cases** (incl. 1c-curriculum exercising §12c through train.py's plumbing) with the non-vacuity criterion; `test_continual_resume_rebuild.py` retargeted at the seam stays green |
...
- **Phase 2**: `test_entry_parity.py` CPU, all three legs — Gate 1a single-config, Gate 1b 2-stage curriculum, Gate 1c same-checkpoint resume in **both cases** (1c-single on the 1a fixture, 1c-curriculum resuming into the 1b fixture's second stage), each subject to the non-vacuity criterion (CI, slow-marked); GPU parametrization manual (1a only); bench_sps A/B; `test_continual_resume_rebuild.py` stays green through the seam refactor.
...
- [ ] `run_dreamer_training` byte-diff vs the old main() body reviewed: only the enumerated mechanical substitutions (spec fields, num_envs/budget deletion with line :593 retained, the two wandb label lines, the log_code call, the two re-derived pre-seam locals, parity guards).
...
- [ ] Gate 1 artifacts reviewed for ALL THREE legs (1a/1b/1c, with 1c in both cases): config-diff masked keys ⊆ compat table, uniform across stages in 1b; CPU bit-identity reports; 1c resumed-counter equality **plus the non-vacuity criterion satisfied** (non-empty telemetry, ≥1 post-`train_start_iter` loss-updating iteration — an instant-exit resume is a FAIL even when both sides match).
