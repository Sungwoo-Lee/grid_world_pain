...
| **Fractional `attack_range` bound silently has no effect** | An `attack_range` of `[2,3]` samples the exact same integer distance as `[2,2]` (you need `[2,4]` to actually widen it by one step) — no warning, no error, matches the project's recurring silent-config-drift theme. | **OPEN — config gotcha** | Low–Med | env config (attack_range) | memory `20260703_0344_attack_range_float_threshold_gotcha` |
