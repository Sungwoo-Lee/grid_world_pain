...
- [ ] Checkpoint 1 — After editing `state.py`, confirm the four new fields are plain `float` leaves (no `struct.field(pytree_node=False)`), so they trace and don't recompile on value change.
- [ ] Checkpoint 2 — `load_env_params` on the unchanged `08-singlePredRabbit_disengage.yaml` (flags `false`, no new keys) still succeeds with no `ValueError`.
- [ ] Checkpoint 3 — A single `jax_reset` from the unchanged `08` config yields `nutrition == 100`, `injury == 0.0` (byte-identical to pre-change).
- [ ] Checkpoint 4 — A reset with `random_start_nutrition: true`, range `[0, 30]` produces a nutrition draw inside `[0, 30]` (and below 50, i.e. in the previously-unreachable hungry band).
- [ ] Checkpoint 5 — Run the full `tests/env/test_unified_parity.py` suite — it must stay **green** (no config gained a mandatory key, so no fixture invalidation). If any parity case flips, STOP and report — it means a non-random-branch path was perturbed.
- [ ] Checkpoint 6 — All six new tests in `test_initial_state_ranges.py` pass.
- [ ] Checkpoint 7 — No new JIT recompilation: changing a range value between two resets with the same config structure must not retrigger compilation (the bounds are traced params, not static).
...
> **Implemented by**: _[developer agent]_
> **Date**: _[pending]_

<!-- Filled by the developer agent after code changes. Describe what was done, deviations, speed before/after. -->
