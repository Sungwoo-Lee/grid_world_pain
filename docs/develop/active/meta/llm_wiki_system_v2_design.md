...
> **Status**: PLANNED
> **Opened**: 2026-05-15
> **Related**: [v1 design](llm_wiki_system_design.md), [.claude-memory/CLAUDE.md operating manual](../../../../.claude-memory/CLAUDE.md), [wiki-write skill](../../../../.claude/skills/wiki-write/SKILL.md), [project CLAUDE.md](../../../../CLAUDE.md)

---

## Context

The project's LLM Wiki layer at `.claude-memory/` (designed in [v1](llm_wiki_system_design.md), 2026-05-08) now holds 56 insights across 6 topic folders and has been load-bearing for three months. Two external ideas published in April 2026 — **Andrej Karpathy's "LLM Wiki" pattern** ([gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)) and **Graphify** ([graphify.net](https://graphify.net/), [github](https://github.com/safishamsi/graphify)) — propose primitives that the v1 design did not pick up. This doc proposes a **v2 upgrade** that adds those primitives to `.claude-memory/`, relocates the layer under `docs/llm_wiki/` so a single Obsidian vault can index everything, and elevates the existing conversation-record pointer (`raw_source` → synced JSONL under `claude_data/`) to a first-class graph node. The existing schema (14-field frontmatter, 5 body sections, 4-layer fragmentation safeguards) stays intact apart from two additive fields. Nothing here is implemented yet — this is the plan for the user to approve before any code or schema changes are made.

The six primitives, in plain English:

0. **Relocate to `docs/llm_wiki/`** — move the entire LLM Wiki from project root `.claude-memory/` to `docs/llm_wiki/` so Obsidian can index `docs/` as a single vault (Obsidian vaults already exist at `./.obsidian` and `./docs/.obsidian`). Single big-bang `git mv` + global path search-replace in one atomic commit; ~40 cross-referencing files need updating. Phase 0 prerequisite for everything else.
...
Re-stated from [v1 design](llm_wiki_system_design.md) and `.claude-memory/CLAUDE.md` for clarity:
...
| Immutable raw / curated wiki separation | JSONL in `claude_data/` (raw) + `entries/<topic>/<id>.md` (curated) | ✅ |
...
| Tag dictionary / entity vocab | `entries/_global_tags.md` | ✅ |
...
8. **Code-side wiki.** `.claude-memory/` is LLM Wiki; there is no Graphify-style index over `src/` that agents can read before answering codebase questions.
...
10. **Layer location blocks single-vault Obsidian use.** The current root-level `.claude-memory/` plus `docs/` split forces two separate Obsidian vaults (the user already has `./.obsidian` and `./docs/.obsidian` configured) — interlinking between session insights and design docs / experiment summaries / diary entries is awkward across vault boundaries. Moving the LLM Wiki under `docs/llm_wiki/` collapses to a single vault and makes Obsidian's native graph view a free byproduct of Phase 1 wikilinks.
...
- **Repo-relative, version-controlled.** All v2 artefacts live under `.claude-memory/` (LLM Wiki) or `graphify-out/` (code layer). `claude_data/` exception still applies for the synced raw JSONLs.
...
#### Feature 0 — Relocate `.claude-memory/` → `docs/llm_wiki/` (Phase 0 prerequisite)

**What.** A single atomic commit that:

1. `git mv .claude-memory docs/llm_wiki` — preserves history; every insight's `git log --follow` still works.
2. Global path search-replace across the ~40 cross-referencing files (full list in **File Changes** below). All occurrences of the literal string `.claude-memory/` → `docs/llm_wiki/` in `*.md`, `*.py`, `*.sh`, `*.json`, `*.yml`, `*.yaml`. Carefully exclude `claude_data/` (the synced JSONL store, unrelated path) and `.claude/worktrees/` (other worktrees' copies, which are their own concern).
...
5. `docs/.obsidian/` (the existing docs vault) auto-picks up the new `docs/llm_wiki/` subtree on next launch — no Obsidian config edit needed unless the user wants to remove the now-redundant root `./.obsidian` vault.

**Why.** Obsidian operates per-vault, and the user already has two competing vaults (`./.obsidian` at root, `./docs/.obsidian` inside docs). Collapsing to a single `docs/` vault means session insights, design docs, diary entries, experiment summaries, and reviews are all interlinkable via Obsidian's native `[[…]]` resolution and graph view. Without this move, Phase 1 wikilinks would only resolve within the LLM Wiki; with this move they resolve across the whole project's documentation surface.

**Folder name choice.** `docs/llm_wiki/` (not `docs/wiki/`, `docs/claude-memory/`, or `docs/.claude-memory/`) — short, semantically explicit, no leading dot (Obsidian shows it without toggling hidden folders), no agent-tool branding (the layer is the project's memory; the agent tool is incidental).
...
- All cross-references from skills (`wiki-write`, `wiki-read`, `diary`, `summarize-study`), scripts (`claude_jsonl_to_md.py`, `diary_append.py`), the v1 design doc, the project root `CLAUDE.md`, and 20+ doc files need string-level updates. Listed in File Changes.
- The v1 design doc gets a `superseded_by: llm_wiki_system_v2_design.md` frontmatter update **only after Phase 0 ships** (until then v1's path references are still correct).
- The `.gitignore` may need an entry for `docs/llm_wiki/.trash/` if `.claude-memory/.trash/` was already ignored.
...
**What.** Allow `[[<insight_id>]]` inline in insight bodies. Write `scripts/regen_wiki_links.py` that walks every insight file, parses `[[…]]` tokens from the body, and rewrites the `related:` frontmatter to the deduplicated sorted list. Run it on every `/wiki-write` commit; also run a one-shot pass to normalise the 56 existing insights' `related:` lines (quoted/unquoted/`[]` → canonical `["<id>", …]` form).
...
**Where it runs.** Same place as `scripts/regen_dev_index.py` runs for `docs/develop/INDEX.md` — wired into `/wiki-write` Step 9 (the auto-commit step), so the graph and backlinks are regenerated and committed in the same atomic commit as the new insight.
...
**What.** Extend `wiki-write` skill Step 4 with a pre-write check. Given a candidate insight with tags `T` and a `## Key conclusion` paragraph `K`:
...
**4a. Lint script.** `scripts/lint_wiki.py` runs the following checks and prints a punch list:
...
Recall (the `wiki-read` skill) should expose `valid_until` in natural-language output: "this finding was marked valid until 2026-08-01; that date has passed — you may want to re-verify."
...
**Why 4c is bundled, not standalone.** It is the only piece in v2 that touches the codebase rather than the LLM Wiki. Bundling reduces the design-doc surface; if 4c is rejected at review, 4a/4b proceed independently.
...
**5a. Conversation node in the graph.** `scripts/regen_wiki_graph.py` (Feature 2) gains a pass that:
...
Insight  : docs/llm_wiki/entries/cluster_ops/20260508_1638_container_slimdown_recipe.md
...
- A session that produced no `/wiki-write`-saved insights is invisible to this layer. That's correct behavior — uninstrumented sessions aren't part of the wiki graph.

### File Changes

For each affected file, the planned change. Note: all path references below assume **Phase 0 has shipped first** — i.e., the layer is at `docs/llm_wiki/`. The Phase 0 relocation manifest is listed separately below.
...
```
git mv .claude-memory docs/llm_wiki
```

`git mv` preserves history; every insight's `git log --follow docs/llm_wiki/entries/<topic>/<id>.md` continues to walk back to the original capture commit.

**Class B — Project-rule files (string replace `.claude-memory/` → `docs/llm_wiki/`)**

| File | Why it references the layer |
|---|---|
| `CLAUDE.md` (project root) | Routes future-Claude to the LLM Wiki |
...
| File | Why it references the layer |
|---|---|
| `.claude/skills/wiki-write/SKILL.md` | Most references (writes to the layer); also `scripts/claude_jsonl_to_md.py` invocation path |
| `.claude/skills/wiki-write/evals/evals.json` | Eval fixtures reference layer paths |
| `.claude/skills/wiki-read/SKILL.md` | Reads the layer |
...
| `docs/develop/active/meta/llm_wiki_system_design.md` (v1) | 1 file | Replace paths; also set `superseded_by: llm_wiki_system_v2_design.md` in frontmatter |

Total: ~40 files. Approach: one `find … -exec sed -i 's|\.claude-memory/|docs/llm_wiki/|g' {} +` invocation scoped to the file globs above, then a manual `git diff --stat` review before commit. Exclusions: `claude_data/` and `.claude/worktrees/*` directories must NOT be touched.
...
#### `docs/llm_wiki/CLAUDE.md` (operating manual, post-Phase-0 path)
...
Add a new sub-step in Step 4: "Before writing: contradiction-check against settled insights with overlapping tags (see CLAUDE.md §13)." Add a new Step 8.5 (between diary and commit): "Run `scripts/regen_wiki_links.py` and `scripts/regen_wiki_graph.py`; stage the touched files into the same commit." Update the Step 9 commit stage list to include `docs/llm_wiki/GRAPH_REPORT.md` and any insight files whose backlink blocks changed.

#### `.claude/skills/wiki-read/SKILL.md`
...
- `scripts/regen_wiki_links.py` — parse `[[id]]` from bodies, rewrite `related:`.
- `scripts/regen_wiki_graph.py` — produce `GRAPH_REPORT.md` (with Conversation provenance section) and per-insight backlink blocks.
- `scripts/lint_wiki.py` — punch-list lint, including `raw_source` resolution.
...
Add `graphify-out/` (Feature 4c). Verify `docs/llm_wiki/.trash/` is ignored (carry over the existing `.claude-memory/.trash/` rule if present).

#### One-shot normalisation pass

A single throwaway script (or a `--normalise` mode on `regen_wiki_links.py`) that walks the 56 existing insights, parses any current `related:` value (handles `[]`, `[id, id]`, `["id", "id"]` forms), rewrites in canonical `["<id>", "<id>"]` form. This is one commit, separate from the v2 ship commit, so any unexpected drift is reviewable.
...
0. **Phase 0 — Relocate `.claude-memory/` → `docs/llm_wiki/`** (Feature 0). Single atomic commit: `git mv` + global sed pass + verification grep gate. All later phases assume `docs/llm_wiki/` as the layer root. Hard prerequisite for all subsequent phases — committing v2 features into the old root location would only have to be unwound later.
...
3. **Phase C — Contradiction flag** (Feature 3). Touches the `wiki-write` skill flow; needs a careful eval (does it false-positive too often on near-duplicate but non-contradictory insights?).
4. **Phase D — Lint** (Feature 4a + 5d). Pure tooling. The `raw_source` resolution checks (5d) ship in the same script.
5. **Phase E — Bitemporal fields** (Feature 4b). Schema change; needs `docs/llm_wiki/CLAUDE.md` + `TEMPLATES/` + `SKILL.md` updates in one commit.
...
**Where Feature 5 (conversation-as-node) lives across phases**: 5a (graph-node extraction) is part of Phase B's `regen_wiki_graph.py`; 5b (provenance section) is part of Phase B's `GRAPH_REPORT.md`; 5c (`open_conversation.py` helper) can ship any time after Phase 0 but is most naturally bundled with Phase B; 5d (lint check) is part of Phase D.
...
- [ ] **Phase 0 — verification grep gate**: after `git mv` + sed pass, `grep -r '\.claude-memory/' --include='*.md' --include='*.py' --include='*.sh' --include='*.json' --exclude-dir='claude_data' --exclude-dir='.claude/worktrees' --exclude-dir='.git' .` returns zero matches. If non-zero, the relocation is incomplete; do not commit. — **FAILED at HEAD** (senior-developer verification, 2026-05-16): `git grep '\.claude-memory/' HEAD -- '*.md'` returns 13 files inside `docs/llm_wiki/` whose internal refs were not updated by the sed pass. Working-tree edits exist but were not committed. Fix-up commit required — see Verification Report for details.
- [x] **Phase 0 — history preservation**: `git log --follow docs/llm_wiki/entries/cluster_ops/<some_id>.md` walks back to the original capture commit (validates `git mv` worked, not a `mv` + `git add`). — **DONE**: `git log --follow docs/llm_wiki/CLAUDE.md` shows 3 commits back to `a16a6c9`.
- [ ] **Phase 0 — Obsidian smoke**: open `docs/.obsidian` vault, confirm `docs/llm_wiki/` appears in the file tree, confirm an existing `related: [<id>]` shows the linked file when clicked (no Phase A wikilinks yet — this just validates Obsidian sees the moved tree). — **requires manual verification by user**.
...
- [x] **Phase A** — after normalisation pass: every existing insight's `related:` value is a YAML list of double-quoted strings; `grep -c '^related:' docs/llm_wiki/entries/*/*.md` returns the expected count; a randomly sampled insight has the same set of related IDs as before the rewrite (parsed both ways). — **DONE**: G1 passes (zero non-canonical lines), 56 files each have exactly one related: line.
- [x] **Phase A** — after `regen_wiki_links.py` ships: write a test insight with `[[<known_id>]]` in the body; run the regenerator; assert the frontmatter `related:` now contains the known id. — **DONE**: G3 synthetic test passes (exit 1 detected, exit 0 after apply).
...
- [x] **Phase B** — `docs/llm_wiki/GRAPH_REPORT.md` has at least the expected sections (God nodes / Orphans / Broken refs / Tag clusters / Surprising connections / Conversation provenance); god-node ranking matches manual inbound-link counts on a sampled insight; broken-ref section is empty after Phase A. — **DONE**: `grep -c '^## ' GRAPH_REPORT.md` = 6; broken-ref section = "None — all wikilinks resolve."
...
- [ ] **Phase E** — write a new insight with explicit `valid_until` and `confidence`; confirm `wiki-read` surfaces a warning when the date is in the past.
...
### Phase 0 — Relocate `.claude-memory/` → `docs/llm_wiki/`
...
**Verification grep result**: Zero matches outside `docs/develop/active/meta/llm_wiki_system_v2_design.md`. All 27 remaining `.claude-memory/` matches are intentional — they describe the v1 historical state in the design doc that was excluded from the sed pass per spec.

**History preservation**: `git log --follow docs/llm_wiki/CLAUDE.md` returns 3 commits, walking back to `a16a6c9` (the original "in-repo LLM Wiki" commit, predating Phase 0 by weeks). All 67 renames used `git mv`, not `mv` + `git add`.

**Deviations from spec**:

1. `.gitignore` not touched by the `find ... -name '*.md' -o -name '*.json' -o -name '*.py' -o -name '*.sh'` sed pass because `.gitignore` has no file extension. Caught and fixed manually: updated `.claude-memory/.trash/*` → `docs/llm_wiki/.trash/*` and `!.claude-memory/.trash/.gitkeep` → `!docs/llm_wiki/.trash/.gitkeep` before commit.

2. The `find ... -not -path '*/docs/develop/active/meta/llm_wiki_system_v2_design.md'` exclusion matched relative paths starting with `./` but not the path without the `./` prefix, causing the v2 design doc to be modified by the initial sed pass. Caught before commit: restored the doc to HEAD via `git checkout HEAD -- <path>` before committing. The committed v2 design doc is identical to its HEAD state (all 27 `.claude-memory/` references preserved).
...
**Commits**:
- `fb9d954` — `scripts/regen_wiki_links.py` (new, 167 lines) + one-time normalisation pass on all 56 insights (25 files updated, 31 already canonical or empty).
- `d3367b0` — `docs/llm_wiki/CLAUDE.md` new §12 + `.claude/skills/wiki-write/SKILL.md` Step 4/9 updates.
...
| Gate | Command | Result |
|---|---|:---:|
| G1 | `grep -h '^related:' docs/llm_wiki/entries/*/*.md \| grep -v '^\[\]$' \| grep -v '^\["'` | ✅ empty |
| G2 | `python scripts/regen_wiki_links.py --check` | ✅ exit 0 |
...
| G5 | `git diff --stat HEAD~2 HEAD~1 docs/llm_wiki/entries/` | ✅ 25 files, each `2 +-` (only `related:` line touched) |
...
`git diff --stat HEAD~2 HEAD~1` shows 110 files, 302 insertions, 301 deletions. With `-M` rename detection 67 files collapse to pure renames (`{.claude-memory => docs/llm_wiki}/...` at 100% similarity, 0 ins / 0 del) — these are the insight files plus the operating-manual-class files inside the moved folder. The remaining 43 files (outside the moved folder) show small symmetric ins/del counts consistent with a path-string sed pass (most files: 2/2, 4/4, 8/8). One large outlier: `docs/develop/active/meta/llm_wiki_system_design.md` (v1) at 131/131 — large but in-scope: this is the primary v1 documentation of the layer, dense with path references, plus the planned `superseded_by:` frontmatter addition.
...
|------|-------|:------:|-------|
| G1 | `git grep '\.claude-memory/'` at HEAD, outside v2 design doc + diary row | ❌ | **BLOCKER.** `git grep -l '\.claude-memory/' HEAD -- '*.md' '*.py' '*.sh' '*.json'` returns 14 files at HEAD. Of these: 1 is the v2 design doc (intentional), and **13 are files INSIDE `docs/llm_wiki/`** whose internal references were not updated by the sed pass. Counts at HEAD: `docs/llm_wiki/CLAUDE.md` (6 refs), `docs/llm_wiki/ROOT_INDEX.md` (5), `docs/llm_wiki/entries/_global_tags.md` (1), `docs/llm_wiki/entries/cluster_ops/20260508_1717_ssh_config_match_user_scoping.md` (1), `docs/llm_wiki/entries/wiki_system_design/{20260508_0315_claude_memory_system_genesis (5), 20260508_0429_memorize_skill_design_and_ship, 20260508_0447_recall_skill_design_and_ship, 20260509_1619_summarize_study_skill_design_and_ship, 20260509_1620_documentation_framing_policy, 20260513_2310_orphan_memory_branch_rewrite, _topic_index (3)}`, `docs/llm_wiki/entries/subagent_engineering/{20260508_0430_worktree_isolation_path_safety (6), 20260509_1621_multi_agent_research_chain_v2_pattern}`. Total residue at HEAD: dozens of stale path strings. The earlier developer-reported "zero matches" check apparently ran against the working tree at a moment when a separate (unrelated, uncommitted) edit pass had already started patching these files — a state that is NOT in either committed commit. |
| G2 | `git log --follow docs/llm_wiki/CLAUDE.md` walks back pre-Phase-0 | ✅ | Returns 3 commits: `6d9f7e9` (today) → `4feaf25` (raw_source pointing at JSONL) → `a16a6c9` (original LLM Wiki commit). `git mv` preserved history. |
| G3 | 4 sentinel files present at new path | ✅ | All exist: `docs/llm_wiki/CLAUDE.md`, `docs/llm_wiki/ROOT_INDEX.md`, `docs/llm_wiki/entries/_global_tags.md`, `docs/llm_wiki/TEMPLATES/entry.md`. |
| G4 | Old `.claude-memory/` does NOT exist | ✅ | `ls .claude-memory/`: `No such file or directory`. |
| G5 | `head -10 docs/llm_wiki/CLAUDE.md` content smell-test against HEAD | ❌ | HEAD's title is still `# CLAUDE.md — \`.claude-memory/\` Operating Manual` (line 1) and body §1 still reads "This layer — `.claude-memory/` —". The path-string sed did not touch this file. Working-tree edits exist but are not committed. |
| G6 | `raw_source:` in insights still points at `claude_data/`, NOT `docs/llm_wiki/` | ✅ | cluster_ops sample: 16 `claude_data/` refs, 0 `docs/llm_wiki/` refs. Sed correctly left `claude_data/` paths alone in insights. (Note: a handful of legacy `_archive/raw_conversations/...` paths from pre-JSONL-sync insights also remain untouched, as expected.) |
| G7 | 6 topic folders under `docs/llm_wiki/entries/` | ✅ | `cluster_ops`, `dreamer_diagnosis`, `hypervigilance`, `wiki_system_design`, `nmn_diagnosis`, `subagent_engineering`. |
| G8 | 56 insight files (excluding `_topic_index.md`, `_global_tags.md`) | ✅ | `find docs/llm_wiki/entries -name '*.md' -not -name '_topic_index.md' -not -name '_global_tags.md' \| wc -l` = 56. |
| G9 | v1 design doc has `superseded_by: llm_wiki_system_v2_design.md` in frontmatter | ✅ | Line 7 of `docs/develop/active/meta/llm_wiki_system_design.md`. Title also updated to `In-repo Session Memory System (docs/llm_wiki/)`. |
...
The Phase 0 sed pass operated on files OUTSIDE the source folder (`.claude/skills/...`, `docs/diary/...`, `docs/develop/...`, `scripts/...`, project-root `CLAUDE.md`, `.gitignore`) and updated their references successfully — the 43 non-rename files in the commit prove this. But the sed pass did not recurse into `.claude-memory/` itself before (or after) the `git mv`. Net result: every internal cross-reference within the LLM Wiki (the operating manual `CLAUDE.md`, the topic registry `ROOT_INDEX.md`, the global tag dictionary, every `_topic_index.md`, and ~10 insight files whose bodies reference the layer's own path) still says `.claude-memory/` at HEAD.
...
1. Stage the existing working-tree edits to the 13 affected files (they appear to already contain the correct sed replacements — verify by re-running `grep -c '\.claude-memory/'` on each working-tree file; the spot checks during verification showed `docs/llm_wiki/CLAUDE.md` working-tree count = 0).
...
3. Commit as `fix(memory): 📦 complete Phase 0 — update internal references inside docs/llm_wiki/` (separate from `6d9f7e9` so the fix is reviewable independently).

### Conclusion

**Phase 0 NOT fully verified — fix-up commit required before Phase A.** G2/G3/G4/G6–G10 (7 gates) pass; G1 and G5 (2 gates) fail because the sed pass left ~30 stale `.claude-memory/` path strings inside the moved folder at HEAD. The fix is mechanical (re-run the same sed inside `docs/llm_wiki/`); the existing uncommitted working-tree edits appear to already contain it. After the fix-up commit, this verification can be re-issued. Until then, Phase A should not start: agents that read `docs/llm_wiki/CLAUDE.md` (the operating manual) at HEAD will see contradictory path guidance.
...
|---|---|:---:|---|
| G1 | `git grep -l '\.claude-memory/' -- '*.md' '*.py' '*.sh' '*.json'` excluding the v2 design doc + today's diary row | ✅ | Zero matches at HEAD. Only intentional historical references in `llm_wiki_system_v2_design.md` (this doc, 27 historical refs) and `docs/diary/2026-05-16.md` (single Phase 0 event row) remain — both acceptable per the spec. |
| G5 | `head -1 docs/llm_wiki/CLAUDE.md` content smell-test | ✅ | Title now `# CLAUDE.md — \`docs/llm_wiki/\` Operating Manual`. §1 body and §2 layer-table rows updated to `docs/llm_wiki/`. |
...
> **Scope**: independent correctness review of `scripts/regen_wiki_links.py`, spot-check of the normalised state, and skill-wiring sanity. Orthogonal to the senior-developer's Verification Protocol (which is plan-adherence-centric); this review checks idempotency, edge cases, and semantic correctness.

#### Plain-language entry point

The developer agent shipped a 167-line stdlib-only Python script that scans every memory insight, finds `[[<insight_id>]]` wikilinks inside the prose body, and rewrites the `related:` frontmatter field into a single canonical sorted-and-quoted form. It then ran the script across all 56 insights, normalising 25 of them. The operating manual (`docs/llm_wiki/CLAUDE.md`) gained a new §12 documenting the conventions, and the `/wiki-write` skill now invokes the regenerator before staging. This review confirms the script is idempotent and edge-case safe, the normalised state passes all five hard gates, no body prose drifted, and the skill wiring is correct.
...
| Multi-line YAML list input (`related:\n  - id1\n  - id2`) | ⚠️ | The script's `RELATED_LINE_RE` matches only the `related:` line itself; if a multi-line block-style list followed, only the first-line value (typically empty after `related:`) would be parsed, and the `- id` lines would be silently treated as body content. **Mitigating fact**: no insight in the corpus uses block-style `related:` (`grep -l '^related:$' docs/llm_wiki/entries/*/*.md` returns no matches; every `related:` in the corpus is single-line inline). So the gap is theoretical for current state, but a future hand-edit that switches to block style would break silently. The spec explicitly called this out as an input form to handle; the implementation does not. Recommend documenting in §12 that block-style `related:` is unsupported, OR extending the parser. Non-blocker because the spec-listed forms `[]`, `[id]`, `[id, id]`, `["id", "id"]` are all handled correctly. |
| `--normalise-existing` vs default mode semantically identical | ✅ | Verified by reading lines 84–86 of the script and the developer's note. The flag is parsed but never branched on; both modes execute the same `process_file` logic. The implementation report calls this out explicitly. This is correct: it means the migration pass and steady-state `/wiki-write` runs use the same code path, so the migration result is by construction idempotent under future runs. |
...
| File-IO safety (atomic write vs partial-write risk) | 🟢 nit | `path.write_text(new_text, encoding="utf-8")` is a single in-place write; no tempfile-then-rename. Acceptable for a script run by a human or by `/wiki-write` Step 9 (not by parallel workers); under SIGKILL mid-write a file could end up truncated. Project has no concurrent-writer model for the LLM Wiki, so the risk is minimal. Flag for awareness. |
...
|---|---|:---:|---|
| G1 | Every `related:` is canonical (`[]` or `["…", …]` form) | ✅ | `grep -h '^related:' docs/llm_wiki/entries/*/*.md \| grep -v '^related: \[\]$' \| grep -v '^related: \["'` returns empty. 56 of 56 insights have exactly one `related:` line. |
| G2 | `--check` is idempotent against HEAD state | ✅ | `python scripts/regen_wiki_links.py --check` prints "OK — no files would change." and exits 0. |
...
| G4 | Body prose did not drift in the normalisation commit | ✅ | `git show fb9d954 -- 'docs/llm_wiki/entries/'` lines filtered to non-`related:` non-header content returned empty. Only the `related:` line in each of the 25 touched files changed. |
...
|---|:---:|---|
| `docs/llm_wiki/CLAUDE.md` new §12 exists with the right content | ✅ | §12 "Wikilinks and graph regeneration" present at lines 230+ of the file; includes Writing wikilinks subsection with `[[<id>]]` syntax, `related:` frontmatter canonical-form table, four-mode CLI table for `regen_wiki_links.py`, the Step 9 contract, and the `[[id\|alias]]` handling note. |
| `docs/llm_wiki/CLAUDE.md` "Last updated" bumped to `2026-05-16` | ✅ | Diff confirms line 6 changed from `2026-05-08` to `2026-05-16`. |
| `.claude/skills/wiki-write/SKILL.md` Step 4 sub-bullet on `[[id]]` usage | ✅ | New bullet at line ~100: "When referencing another insight, write `[[<insight_id>]]` inline in the body section… The `related:` frontmatter is auto-populated by `scripts/regen_wiki_links.py` — do not hand-type it." |
| `.claude/skills/wiki-write/SKILL.md` Step 9 pre-stage regen invocation | ✅ | New paragraph + fenced command block in Step 9: `python scripts/regen_wiki_links.py` runs before `git add`. Conditional sentinel ("If regen_wiki_links.py updated older insights, add those paths too") added to the `git add` block. |
...
|---|---|---|:---:|
| `fb9d954` | `scripts/regen_wiki_links.py` (new) + `docs/llm_wiki/entries/**/*.md` (normalised) | 1 new script + 25 insight files across 6 topic folders; no other paths touched | ✅ |
| `d3367b0` | `docs/llm_wiki/CLAUDE.md` + `.claude/skills/wiki-write/SKILL.md` | Exactly those two files (`+57/-1`); no other paths touched | ✅ |
| `587e31f` | Implementation report append to `docs/develop/active/meta/llm_wiki_system_v2_design.md` | Single file (`+42/-3`); within scope | ✅ |
...
> **Date**: 2026-05-16
> **Commits**:
> - `5500ec2` — scripts/regen_wiki_graph.py (622 lines) + scripts/open_conversation.py (226 lines)
> - `fe26f0c` — docs/llm_wiki/GRAPH_REPORT.md (new) + Backlinks blocks injected into all 56 insights (57 files)
> - `7053da5` — .claude/skills/wiki-write/SKILL.md Step 9 wiring

**Summary of what was implemented (file-by-file)**:

1. `scripts/regen_wiki_graph.py` (622 lines, stdlib-only, executable):
   - Walks `docs/llm_wiki/entries/`, parses frontmatter + body, builds `InsightNode` and `SessionNode` in-wiki graph.
...
3. `.claude/skills/wiki-write/SKILL.md` Step 9:
   - Added second regenerator call (`regen_wiki_graph.py`) after `regen_wiki_links.py`.
   - Added `docs/llm_wiki/GRAPH_REPORT.md` to the staging list.
...
| G3 | Section count in GRAPH_REPORT.md | ✅ `grep -c '^## ' docs/llm_wiki/GRAPH_REPORT.md` = 6 |
...
**Speed check**: N/A — scripts/regen_wiki_graph.py is a one-shot text tool; no training hot path or model code touched.
...
> **Scope**: independent correctness review of `scripts/regen_wiki_graph.py` (622 lines), `scripts/open_conversation.py` (226 lines), the generated `docs/llm_wiki/GRAPH_REPORT.md`, and `/wiki-write` Step 9 wiring. `claude_data/` is gitignored and absent from this worktree — graceful-degradation mode is the path under test for the Conversation provenance section.

#### Plain-language summary

Phase B promotes the v1 `related:` frontmatter graph into a full link-graph layer with (a) a top-level `GRAPH_REPORT.md` overview, (b) per-insight Backlinks blocks delimited by HTML-comment markers, and (c) a `scripts/open_conversation.py` helper that resolves any insight or session UUID back to its raw conversation log. The regenerator is stdlib-only, idempotent, and wired into `/wiki-write` Step 9. This review independently re-ran every gate, manually recomputed the edge count, sampled god-node / orphan / surprising-connection sections against the file system, exercised all three exit paths of `open_conversation.py`, and inspected the script for the three follow-ups Phase A flagged. All gates pass; the self-link follow-up from Phase A is correctly implemented; no out-of-scope file changes; the timestamp-only idempotency carve-out is a sensible interpretation of the spec.

#### Feature audit — `scripts/regen_wiki_graph.py`

| # | Aspect | Verdict | Notes |
|---|---|:---:|---|
| 1 | Graph construction: insight nodes from `docs/llm_wiki/entries/<topic>/<id>.md`, excluding `_topic_index.md`, `_global_tags.md`, `_archive/`, `.trash/` | ✅ | `collect_insights()` lines 148-163; manual `find` returns 56, the script's main loop produces 56 insight nodes. |
...
|---|---|---|---|:---:|
| G1 | `python scripts/regen_wiki_graph.py` initial run | files written | (history) 57 files written initially | ✅ |
| G2 | `python scripts/regen_wiki_graph.py --check` | exit 0 | "OK — no files would change." exit=0 | ✅ |
| G3 | `grep -c '^## ' docs/llm_wiki/GRAPH_REPORT.md` | 6 | 6 | ✅ |
| G4a | `find docs/llm_wiki/entries -name '[0-9]*.md' \| wc -l` | 56 | 56 | ✅ |
| G4b | `grep -l 'BACKLINKS' docs/llm_wiki/entries/*/[0-9]*.md \| wc -l` | 56 | 56 | ✅ |
...
|---|:---:|:---:|
| `scripts/regen_wiki_graph.py` (new) | 1 | ✅ |
| `scripts/open_conversation.py` (new) | 1 | ✅ |
| `docs/llm_wiki/GRAPH_REPORT.md` (new) | 1 | ✅ |
| `docs/llm_wiki/entries/**/*.md` (Backlinks blocks) | 56 | ✅ — exactly the universe of insights |
| `.claude/skills/wiki-write/SKILL.md` (Step 9 wiring) | 1 | ✅ |
| `docs/develop/active/meta/llm_wiki_system_v2_design.md` (Impl Report) | 1 | ✅ |
...
| Atomic write in `regen_wiki_links.py` | 🟢 nit | N/A for Phase B (reviewer scoped this to Phase D). |
...
**Issue (found in Phase C verification)**: Phase B's `<!-- BACKLINKS … -->` block injection caused `regen_wiki_links.py` to pick up the `[[id]]` tokens inside those blocks as new outbound wikilinks, polluting `related:` frontmatter. `--check` exited 1 with 16 files that would be incorrectly modified. Phase B's `regen_wiki_graph.py` already stripped BACKLINKS blocks before scanning (lines 206-211) but `regen_wiki_links.py` did not.

**Fix**: Added `BACKLINKS_BLOCK_RE` regex stripping in `scripts/regen_wiki_links.py` (12-line change, mirroring the same exclusion). All four verification gates pass: G1 (`--check` exits 0), G2 (no diff after full run), G3 (`regen_wiki_graph.py --check` still exits 0), G4 (real body wikilinks still detected).
...
> **Implemented by**: developer (Claude Sonnet 4.6)
> **Date**: 2026-05-16
> **Commit**: `d23cf71` — docs/llm_wiki/CLAUDE.md + .claude/skills/wiki-write/SKILL.md

#### Summary of what was implemented (file-by-file)

1. **`docs/llm_wiki/CLAUDE.md`** (+43 lines): new `## 13. Contradiction handling at ingest` section after §12, containing the full Y/S/N user-prompt contract, tag-overlap scope rules, failure mode analysis, and the "why no separate script" rationale. "Last updated" already read `2026-05-16` from Phase A/B.

2. **`.claude/skills/wiki-write/SKILL.md`** (+12 lines): new `### Step 4.5 — Contradiction check (before each write)` sub-step inserted between the timestamp-compute step (item 1) and the file-write step (item 2) within Step 4. Points at [CLAUDE.md §13](../../../docs/llm_wiki/CLAUDE.md#13-contradiction-handling-at-ingest) for the full contract. Cross-link path `../../../docs/llm_wiki/CLAUDE.md` verified to resolve correctly from `.claude/skills/wiki-write/`.

#### Verification gates

| Gate | Command | Result | Notes |
|---|---|:---:|---|
| G1 | `grep -A 5 '^## 13\.' docs/llm_wiki/CLAUDE.md \| head -10` | ✅ | §13 header + opening prose present |
| G2 | `grep -B 1 -A 5 'Step 4.5' .claude/skills/wiki-write/SKILL.md \| head -10` | ✅ | Step 4.5 present with correct content |
| G3 | `test -f .claude/skills/wiki-write/../../../docs/llm_wiki/CLAUDE.md && echo OK` | ✅ | Cross-link path resolves |
| G4 | False-positive eval (see below) | ✅ informational | pair count well under 500 hard-stop threshold |
| G5a | `python scripts/regen_wiki_links.py --check` | ❌ pre-existing | 16 files would change — **PRE-EXISTING Phase B issue** (see Deviation 1) |
| G5b | `python scripts/regen_wiki_graph.py --check` | ✅ | "OK — no files would change." exit 0 |
...
1. **G5a (regen_wiki_links --check) exits 1 at HEAD before Phase C.** This is a pre-existing Phase B issue: Phase B injected `<!-- BACKLINKS … -->` blocks containing `[[id]]` tokens into all 56 insight files, but `regen_wiki_links.py` does NOT strip BACKLINKS blocks before scanning the body for wikilinks. On the second run (post-Phase B), `regen_wiki_links.py` detects BACKLINKS-block tokens as new outbound links and tries to add them to `related:`. Phase C does not cause this — the failure was already present at HEAD (`3b053bf`) before any Phase C edits. Confirmed by `git stash` + `--check` reproducing the same 16-file exit-1. **Recommendation**: Phase B's `regen_wiki_links.py` should be patched to strip BACKLINKS blocks before scanning (same logic Phase B's `regen_wiki_graph.py` already applies). Flag for senior-developer review; not a Phase C deliverable.
...
| Cross-link path `../../../docs/llm_wiki/CLAUDE.md` | ✅ | `readlink -f` from `.claude/skills/wiki-write/` resolves to the actual operating manual. Path is correct relative to the SKILL file location. |
...
| Item | Status | Notes |
|---|:---:|---|
| Root cause is genuinely Phase B, not Phase C | ✅ | Phase B's BACKLINKS-block injection happened in `5500ec2`/`fe26f0c` (Phase B implementation). `regen_wiki_links.py` originated in Phase A (`fb9d954`) and never accounted for blocks that wouldn't exist until Phase B. The bug was latent until Phase B ran, and Phase C just made running `--check` part of the verification gates. The fix correctly belongs in `scripts/regen_wiki_links.py`. |
...
| Both `--check` exit 0 at HEAD | ✅ | Re-ran independently: `regen_wiki_links.py --check` → "OK — no files would change." exit 0. `regen_wiki_graph.py --check` → "OK — no files would change." exit 0. |
...
|---|---|---|:---:|
| `d23cf71` (§13 + Step 4.5) | `docs/llm_wiki/CLAUDE.md`, `.claude/skills/wiki-write/SKILL.md`, `docs/develop/active/meta/llm_wiki_system_v2_design.md` | Exactly those three: +43, +12, +69 lines (Implementation Report appendage). | ✅ |
| `1474149` (implementation report) | folded into `d23cf71` above per the diff stat range | (combined commit shown in the +69 above) | ✅ |
| `33f98b6` (hotfix) | `scripts/regen_wiki_links.py` only | +12 / -2 in one file. | ✅ |
| `12b85d4` (Phase B note) | `docs/develop/active/meta/llm_wiki_system_v2_design.md` only | +8 / -0. | ✅ |
...
The developer agent timed out at tool-use 31 with the script written and `docs/llm_wiki/CLAUDE.md` edited but neither committed, plus one stray uncommitted edit to `docs/llm_wiki/entries/cluster_ops/20260508_1717_ssh_config_match_user_scoping.md` (a single trailing blank-line — likely a synthetic-test residue that was not fully reverted before the timeout). Top-level Claude:
...
Phase D shipped one read-only audit script (`scripts/lint_wiki.py`, 613 lines, stdlib only) and one one-line wiring update in the LLM Wiki's operating manual at `docs/llm_wiki/CLAUDE.md` §6 layer 4 that points the audit trigger at the new script. The script runs nine checks: broken `[[id]]` wikilinks, broken `related:` IDs, `folder:` frontmatter ↔ parent-directory mismatch, duplicate `id:` across the tree, tags missing from the global tag dictionary, near-duplicate folder definitions in the topic registry, old orphan insights (no links, > 30 days), broken `raw_source` JSONL pointers, and a reverse-check that every referenced JSONL appears in the auto-generated graph report's Conversation provenance section. Verification re-ran the developer's G3 synthetic-edit gate that top-level Claude only verified by code inspection, plus independently exercised G4–G6 plus a defensive G3 on a fresh sample, plus exercised the unit-tested-but-not-gated Checks 2, 6, and 7. All synthetic-edit gates fire correctly; the Phase A + B regenerators still report clean trees; no out-of-scope edits in the Phase D commit set. **Verdict: proceed to Phase E**, with two non-blocking nits documented below.

#### 1. Per-check code-correctness audit (Checks 1–9)

Audit performed by reading `scripts/lint_wiki.py` end-to-end and cross-referencing the BACKLINKS-strip regex against `scripts/regen_wiki_links.py` and `scripts/regen_wiki_graph.py`.

| # | Check | Audit verdict | Notes |
|---|-------|:---:|-------|
| 1 | Wikilink `[[id]]` resolution | ✅ | `BACKLINKS_BLOCK_RE = re.compile(r"<!-- BACKLINKS.*?<!-- END BACKLINKS -->\s*", re.DOTALL)` at lint_wiki.py:26 is byte-identical to the regex in `regen_wiki_links.py:25`. The Phase A hotfix logic is mirrored exactly. `ID_PATTERN` (`r"\[\[(\d{8}_\d{4}_[a-z0-9_]+)(?:\|[^\]]+)?\]\]"`, line 23) correctly excludes coordinate arrays `[[1,1],[5,5]]` and handles the alias form `[[id\|alias]]`. |
...
| 5 | Tags in `_global_tags.md` | ✅ | `load_global_tags` (lines 365–387) reads only the rows between `## Active tags` and the next `## ` heading, and skips the `Tag` header row. The `TAG_ROW_RE = re.compile(r"^\|\s*\`([^\`]+)\`\s*\|")` matches the actual table format in `entries/_global_tags.md`. Robust to extra columns; not robust to backtick-less entries (none exist). |
...
**Read-only**: grep `write_text\|write(\|os\.remove\|shutil\|rename\|unlink` in `scripts/lint_wiki.py` returns zero matches. ✅
...
| G6' (folder-def overlap) | Edited `ROOT_INDEX.md` to make `cluster_ops` definition identical to `wiki_system_design` | Exit 1, Check [6] flags overlap 1.00 ≥ 0.7 | Exit 1, Check [6] flagged exactly that with both folder names and definitions | ✅ |
...
| G7 (regenerators clean) | None — re-ran `regen_wiki_links.py --check` and `regen_wiki_graph.py --check` after all G3–G6 reverts | Both exit 0 | links exit 0, graph exit 0 | ✅ |

#### 3. Operating-manual wiring sanity

| Check | Status | Detail |
|---|:---:|---|
| `docs/llm_wiki/CLAUDE.md` §6 layer 4 names `scripts/lint_wiki.py` | ✅ | Line 132 reads: "run `scripts/lint_wiki.py` to produce the full punch list (broken refs, orphans, tag-dictionary drift, near-duplicate folder definitions, `raw_source` resolution)". |
...
| "Last updated" bumped to 2026-05-16 | ✅ | `docs/llm_wiki/CLAUDE.md` line 6: `**Last updated**: 2026-05-16`. |
...
| File | Lines | In-scope? |
|---|---|:---:|
| `scripts/lint_wiki.py` | +613 (new) | ✅ (Phase D Feature 4a + 5d) |
| `docs/llm_wiki/CLAUDE.md` | +1/-1 | ✅ (§6 layer 4 wiring) |
| `docs/develop/active/meta/llm_wiki_system_v2_design.md` | +45 | ✅ (Phase D implementation report) |
...
|---|---|---|---|
| 🟢 nit | `scripts/lint_wiki.py:572` (`warnings_only` computation) | The third disjunct `(not issues_reverse or issues_reverse[0].strip().startswith("claude_data/"))` evaluates to `True` even when there are zero warnings (because `not [] == True`). Combined with `genuine_errors == False`, the summary line then reads `"Summary: 0 errors. 0 warning(s) (see above)."` instead of the more pleasant `"Summary: ✅ All checks passed — LLM Wiki is clean."` reachable only via the final `else` branch. Cosmetic only — exit code is correct (0). | Tighten the `warnings_only` predicate to require `total_warnings > 0`, or restructure the summary as a single `if total_warnings == 0 and not genuine_errors` clean-branch. Optional Phase E cleanup. |
| 🟢 nit | `scripts/lint_wiki.py:17` (`TODAY = date(2026, 5, 16)` hardcoded) | The canonical date is hardcoded rather than using `date.today()`. This means Check 7 (orphan-age) ages will become stale as time passes. Defensible for reproducibility during Phase D verification, but in production the script will under-flag orphans as the calendar moves forward. | Switch to `date.today()` in Phase E or add a `--today YYYY-MM-DD` CLI override for reproducible audits. The 2026-05-16 anchor is explicitly per-spec, so this is by design at landing, not a defect. |
...
| Check | Result |
|---|:---:|
| BACKLINKS-strip regex matches `regen_wiki_links.py` / `regen_wiki_graph.py` | ✅ |
...
- v1 design: [llm_wiki_system_design.md](llm_wiki_system_design.md) — to be marked `superseded_by` this doc only after Phase 0 ships.
- Operating manual (current path; relocates to `docs/llm_wiki/CLAUDE.md` after Phase 0): [.claude-memory/CLAUDE.md](../../../../.claude-memory/CLAUDE.md)
- Memorize skill: [.claude/skills/wiki-write/SKILL.md](../../../../.claude/skills/wiki-write/SKILL.md)
- Recall skill: [.claude/skills/wiki-read/SKILL.md](../../../../.claude/skills/wiki-read/SKILL.md)
...
| File | Change |
|---|---|
| `docs/llm_wiki/CLAUDE.md` | §5 template: 16-field header + `valid_until` + `confidence` added after `status:`; full field-semantics list added (15 bullets covering all 16 fields — `date`/`time` share one bullet per pre-existing style); §8 "Do not expose" updated with `valid_until` expiry warning exception. |
| `docs/llm_wiki/TEMPLATES/entry.md` | Added `valid_until: null` and `confidence: null` after `status:` line. Template is now 16 fields. |
| `.claude/skills/wiki-write/SKILL.md` | Description updated `14-field` → `16-field`; Quick-reference table updated `14 fields` → `16 fields` with full field list; Step 4 sub-bullet added for `valid_until` + `confidence` guidance. |
| `.claude/skills/wiki-read/SKILL.md` | Hard-rules section: new `valid_until` expiry warning rule (exception to "do not expose frontmatter"). Step 4 (drill-down): added post-render check for expired `valid_until` with warning line. |
| `scripts/lint_wiki.py` | Phase D nit #2 fixed: `TODAY = date(2026, 5, 16)` → `TODAY = date.today()` with `--today YYYY-MM-DD` CLI override. `Insight` class extended with `valid_until` + `confidence` slots. Check 10 (`valid_until` must be YYYY-MM-DD or null/absent) and Check 11 (`confidence` must be `high`/`medium`/`low`/`null`/absent) added. TOTAL bumped 9→11. Both JSON and human-readable output paths updated. Summary error-counting updated. |
...
|---|---|---|
| G1 | Field-semantics bullets in `docs/llm_wiki/CLAUDE.md` cover all 16 fields | ✅ 15 bullets (date+time share one, per existing style); both `valid_until` and `confidence` bullets present |
| G2 | `grep -c` for `valid_until:` and `confidence:` in `TEMPLATES/entry.md` | ✅ 2 |
| G3 | `regen_wiki_links.py --check` | ✅ exit 0 ("OK — no files would change") |
| G3 | `regen_wiki_graph.py --check` | ✅ exit 0 ("OK — no files would change") |
| G4 | `lint_wiki.py` full run on 56 existing insights | ✅ exit 0, checks [10/11] and [11/11] both pass |
...
3. `e655424` — `feat(memory): ✨ Phase E — lint_wiki.py learns valid_until + confidence checks`
...
| Item | Check | Result |
|---|---|:---:|
| `docs/llm_wiki/CLAUDE.md` §5 template block | `valid_until` and `confidence` present, placed after `status:` (lifecycle-adjacent) | ✅ |
...
| `docs/llm_wiki/TEMPLATES/entry.md` | Skeleton gains `valid_until: null` and `confidence: null` (correct backward-compatible default) | ✅ |
| Backward compatibility | `regen_wiki_links.py --check` exit 0, `regen_wiki_graph.py --check` exit 0, `lint_wiki.py` exit 0 on 56 existing pre-E insights | ✅ |
...
#### 3. Lint extension audit (`scripts/lint_wiki.py`)
...
|---|---|---|---|:---:|
| G1 | `grep -c '^[a-z_]*: ' docs/llm_wiki/TEMPLATES/entry.md` | 16 fields | 16 | ✅ |
| G2a | `regen_wiki_links.py --check` | exit 0 | `OK — no files would change.` exit 0 | ✅ |
| G2b | `regen_wiki_graph.py --check` | exit 0 | `OK — no files would change.` exit 0 | ✅ |
| G3 | `lint_wiki.py` on HEAD; count `^[` headers | exit 0; 11 headers | exit 0; 11 headers; `Summary: 0 errors. 2 warning(s)` (pre-existing `claude_data/` skip) | ✅ |
...
| G7a | `lint_wiki.py --today 2027-01-01` | exit 0; header shows override date | exit 0; header line `docs/llm_wiki/ lint report — 2027-01-01 03:22` | ✅ |
| G7b | `lint_wiki.py --today bogus` | exit 2 | exit 2; `Error: --today 'bogus' is not a valid YYYY-MM-DD date.` | ✅ |
...
|---|---:|---:|:---:|
| `docs/llm_wiki/CLAUDE.md` | +15 | −3 | ✅ §5 template + semantics + §8 carve-out |
| `docs/llm_wiki/TEMPLATES/entry.md` | +2 | −0 | ✅ |
| `.claude/skills/wiki-write/SKILL.md` | +4 | −3 | ✅ description, quick-ref table, Step 4 bullet |
| `.claude/skills/wiki-read/SKILL.md` | +4 | −1 | ✅ Hard-rule + Step 4 post-render |
| `scripts/lint_wiki.py` | +78 | −3 | ✅ Checks 10+11, `--today`, TOTAL bump |
| `docs/develop/active/meta/llm_wiki_system_v2_design.md` | +63 | −0 | ✅ Implementation Report |

No out-of-scope files modified.

#### 6. Deviation assessment

- **G1 grep count = 15 not 16.** Developer's report says 15 bullets because `date` and `time` share a single bullet ("the capture date ... the capture time ..."). This is consistent with the pre-existing §5 style (combining date/time was already the convention before Phase E), and all 16 fields are accounted for in the semantics list. **Accepted — not a regression.** Note: my own G1 measured 16 against `TEMPLATES/entry.md` (one line per field including `date` and `time` separately), which is the gate the brief actually specified — that gate passes cleanly at 16.
...
Not applicable. No hot-path code changed. The lint script is invoked manually (Step 9 of `wiki-write`, ad-hoc audit); the new checks add two single-pass iterations over the in-memory insight list, well below any measurable threshold. Developer correctly skipped speed measurement.

#### Conclusion

**Phase E verified — proceed to Phase F.** All four commits implement the spec faithfully; all seven hard-gates (G1–G7) fire with the expected exit codes and diagnostics; the 56 pre-E insights remain backward-compatible (regenerators and lint all exit 0); skill wiring on both `wiki-write` and `wiki-read` correctly references the 16-field schema and the new `valid_until` recall warning; the Phase D nit #2 (hardcoded `TODAY`) is fixed with a `--today` CLI override; no out-of-scope edits. Two deviations are documented and accepted (combined `date/time` bullet, additive semantics for 9 pre-existing fields).
...
| G4a | `regen_wiki_links.py --check` | ✅ exit 0 |
| G4b | `regen_wiki_graph.py --check` | ✅ exit 0 |
| G4c | `lint_wiki.py > /dev/null 2>&1` | ✅ exit 0 |
...
|---|---|---|---|
| 0 | Relocate `.claude-memory/` → `docs/llm_wiki/` | `6d9f7e9` (reloc) + `fa5d0c5` (fix-up) | ✅ Verified (re-verified after fix-up) |
...
| D | Lint script (`lint_wiki.py`) | `1408672` (script) + `7c9c034` (manual wiring) | ✅ Verified |
...
| `docs/develop/active/meta/llm_wiki_system_v2_design.md` | Implementation Report + checkpoint update for Phase F. | +76 lines: Phase F implementation report subsection, deviations, speed check, commit-summary table. Phase F checkpoint marked `[x] DONE` at line 373. | ✅ |
...
| G3-links | `regen_wiki_links.py --check` | exit 0 | ✅ "OK — no files would change" |
| G3-graph | `regen_wiki_graph.py --check` | exit 0 | ✅ "OK — no files would change" |
| G3-lint | `lint_wiki.py` | exit 0 | ✅ exit 0, 0 errors, 2 warnings (both pre-existing `claude_data/ not present locally` notices — unrelated to Phase F) |
...
|---|---|---|---|---|
| 0 | Relocate `.claude-memory/` → `docs/llm_wiki/` | `6d9f7e9` (+ `fa5d0c5` fix-up) | `458aa4d` | ✅ |
