...
A reference implementation of a richer memory system already exists at `tmp/Claude-memory/` (Korean operating manual, three topic folders, twelve insights, fragmentation safeguards, lazy-load levels, raw-conversation archive). It belongs to a different project but the design is sound. We want a port of the reference's **memory layer** into this repo, in English, version-controlled, **coexisting** with the built-in auto-memory rather than replacing it.
...
1. **3-tier hierarchy**: `CLAUDE.md` (operating manual) → `ROOT_INDEX.md` (topic-folder registry) → `memories/<topic>/_topic_index.md` + per-insight files.
...
8. **Global tag dictionary** at `memories/_global_tags.md` to suppress tag drift.
...
- **No initial topic folders.** Seed only creates `_global_tags.md`, the `TEMPLATES/insight.md`, and an empty `memories/` plus `_archive/raw_conversations/`. The first real insight creates the first topic folder.
...
3. New-session flow (read this file → on memory work, also read `ROOT_INDEX.md` and `_global_tags.md`).
...
| `docs/develop/active/meta/claude_memory_system_design.md` | Updated Checkpoints (ticked off) and this Implementation Report |
...
- Genesis insight References section links to plan doc (`docs/develop/active/meta/claude_memory_system_design.md`), root `CLAUDE.md` (Session memory section), built-in MEMORY.md, and helper script. ✅
