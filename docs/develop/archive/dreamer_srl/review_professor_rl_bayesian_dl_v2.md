...
| 5 | Missing `distribution.type` and `distribution.validate_args` keys in YAML | ✅ RESOLVED | `configs/models/dreamer_srl/agent_xs.yaml` lines 401–405 (new top-level `distribution:` block) | New top-level block exactly as proposed: `type: "auto"` + `validate_args: false`, with sheeprl source line cited. Also surfaces in Risks §10 mandatory-key audit (line 801: "`distribution.type`, `distribution.validate_args`"). |
