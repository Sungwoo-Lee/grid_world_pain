...
- **D'Oro et al. (2023), "Breaking the Replay Ratio Barrier"** — Turns the reset idea into a *scaling* lever. The **replay ratio** is how many gradient updates the agent makes per environment step; cranking it up (more learning from the same data) normally makes agents plateau or collapse from plasticity loss / primacy bias — the "replay-ratio barrier." Fix: periodically **reset the agent's networks (fully or partially) while keeping the replay buffer** — the same reset move as Primacy Bias (Nikishin 2022), now shown to be the *enabling ingredient* that lets the replay ratio be pushed up by an order of magnitude. Substantially improves sample efficiency on Atari 100k and the DeepMind Control Suite. ICLR 2023 oral (top 5%). Directly relevant to our own replay-ratio speed-vs-performance study [[replay_ratio_speed_vs_performance]].
...
- *High-replay-ratio resets* (D'Oro 2023) — periodic full/partial resets are precisely what *enable* scaling the replay ratio (updates per env step) by ~10×; without them, high replay ratios collapse. Targets the primacy-bias / plasticity loss that otherwise caps how hard you can train on a fixed data budget.
...
| 23 | **Sample-Efficient Reinforcement Learning by Breaking the Replay Ratio Barrier.** Pierluca D'Oro, Max Schwarzer, Evgenii Nikishin, Pierre-Luc Bacon, Marc G. Bellemare, Aaron Courville. *ICLR* 2023 (Oral, top 5%). | OpenReview: <https://openreview.net/forum?id=OpC-9aBBVJe> · PDF: <https://openreview.net/pdf?id=OpC-9aBBVJe> · Code: <https://github.com/proceduralia/high_replay_ratio_continuous_control> · DBLP: <https://dblp.org/rec/conf/iclr/DOroSNBBC23.html>. See §6.5 — no arXiv version; OpenReview is the source of record, **flagged**. |
...
5. **#23 D'Oro et al. (2023), "Breaking the Replay Ratio Barrier"** — Title, all six authors, and ICLR 2023 oral (top-5%) venue web-confirmed **2026-07-13** (added after the 2026-06-24 sweep). **There is no arXiv version**; the paper lives only on OpenReview, whose `forum`/`pdf` endpoints are behind a bot-challenge wall (a plain fetch returns a verification page, not the PDF), so download it via a browser session or the authors' code repo rather than a scripted pull. A NeurIPS 2022 Deep RL Workshop version also exists. *Action: cite the ICLR 2023 version; expect the OpenReview PDF link to require interactive access.*
