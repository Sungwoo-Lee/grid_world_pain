...
- Tooling that already exists is sufficient — no script work needed:
  ...
  - `scripts/regen_wiki_links.py` and `scripts/regen_wiki_graph.py` already handle the wiki tree's wikilinks; the broader doc trees rely entirely on Foam's runtime resolution.
