...
summary: "The in-repo `docs/llm_wiki/` layer coexists with the built-in auto-memory MEMORY.md; the two layers split by insight density — short typed rules stay in the built-in layer, multi-section session insights go in `docs/llm_wiki/`. Raw conversation archives are local-only and gitignored."
related: []
...
# Claude LLM Wiki: design decisions and genesis

## Key conclusion

The in-repo `docs/llm_wiki/` layer is created alongside (not replacing) the Claude Code built-in auto-memory at `~/.claude/.../memory/MEMORY.md`. The two layers split by **insight density**: short typed rules that any agent must obey on every invocation stay in the built-in layer; multi-section session insights with rationale, decisions, follow-ups, and raw-conversation traceability go in `docs/llm_wiki/`. Raw conversation archives are local-only (gitignored by default) — the insight file (with `raw_source` field) is committed; the archive itself is not.

## Evidence, measurements, facts

- Design plan: `docs/develop/active/meta/llm_wiki_system_design.md` (status: active, created 2026-05-08).
...
- Four user decisions recorded in the plan (resolved 2026-05-08):
  ...
  - **Location**: `docs/llm_wiki/` at repo root.
...
- Five open questions resolved by the user before implementation:
  ...
  3. **Slash command `/wiki-write`**: no custom Claude Code slash-command wiring; treat `/wiki-write` and other phrases as plain natural-language trigger phrases.
...
- Created `docs/llm_wiki/` skeleton at repo root: `CLAUDE.md`, `ROOT_INDEX.md`, `entries/_global_tags.md`, `TEMPLATES/entry.md`, `_archive/raw_conversations/.gitkeep`, `.trash/.gitkeep`, `entries/wiki_system_design/_topic_index.md`, and this genesis insight file.
...
- Appended "Session memory" section to project root `CLAUDE.md` routing future-Claude to `docs/llm_wiki/CLAUDE.md` and `ROOT_INDEX.md`.
- Added `.gitignore` rules: `_archive/raw_conversations/*.md` (with `!.gitkeep` exception) and `.trash/*` (with `!.gitkeep` exception).
- `ROOT_INDEX.md` populated with `wiki_system_design` folder row (1 insight, top tags: `[wiki, design, decision]`).
...
- Plan doc: `docs/develop/active/meta/llm_wiki_system_design.md`
...
- Why a new folder: first insight in this layer; "wiki_system_design" mirrors the reference layout's same-named topic and is the natural home for any future insight about this layer's own evolution. No existing folder to match against (this is the first folder).
