...
summary: "Designed and shipped the /wiki-write Claude Code skill (.claude/skills/wiki-write/) that captures conversations into the in-repo docs/llm_wiki/ layer; validated end-to-end via a 3-eval benchmark with +33pt with-vs-without skill delta."
related: ["20260508_0315_claude_memory_system_genesis"]
session_origin: claude_code
session_label: "wiki-write skill rollout"
importance: high
status: settled
supersedes: []
...
# `/wiki-write` skill design and ship

## Key conclusion

Built `.claude/skills/wiki-write/` — a project-local Claude Code skill that captures the current conversation as one or more 5-section insight files in the `docs/llm_wiki/` layer, updates the topic and global indexes, and optionally archives the raw conversation. Shipped at commit `f878873`. The skill is now the canonical capture path (Step-by-step flow in `docs/llm_wiki/CLAUDE.md` §4); future `/wiki-write` invocations use it.

## Evidence, measurements, facts

- **Skill location**: `.claude/skills/wiki-write/SKILL.md` (project-local, ships with the repo).
- **Decisions captured at design-time** (locked via AskUserQuestion):
  - Name: `wiki-write` (mirrors the operating-manual trigger phrase "memorize this").
...
- **Eval results (iteration-1, 3 evals × 2 configs = 6 subagent runs in worktrees)**:
  ...
  - eval-2 multi-insight: with_skill 2/10 (20%, **timed out at 6 min before substantive writes**), without_skill 8/10 (80%, but the agent wrote to the main repo's `docs/llm_wiki/` and to `~/.claude/.../memory/MEMORY.md` — boundary violation, see paired insight `20260508_0430_worktree_isolation_path_safety`).
...
- **Iteration-2 changes applied directly** (user opted to skip a second eval round): added a quick-reference table at the top of SKILL.md (saves time + tokens by avoiding re-reads of `docs/llm_wiki/CLAUDE.md` for every step), explicit "repo-relative paths only" hard rule (prevents the boundary-violation pattern observed in the eval-2 baseline), loosened the eval-1 assertion from "exactly one insight" to "one or two" (the agent's split of scope-decision from supporting pilot finding was defensible).
- **Tooling shipped**: `.claude/skills/wiki-write-workspace/grade.py` (programmatic assertion grader), `.claude/skills/wiki-write-workspace/build_benchmark.py` (turns grading.json files into benchmark.json). Workspace dirs gitignored as transient.
- **Coexistence rule** (from operating manual §2): short typed rules another agent must obey on every invocation → built-in `~/.claude/.../memory/MEMORY.md`; multi-section session insights → `docs/llm_wiki/`. The skill enforces this by hard rule and never writes to MEMORY.md.

## Decisions and actions

- Shipped `.claude/skills/wiki-write/` with `SKILL.md` and `evals/evals.json`.
...
- Did **not** wire `/wiki-write` as a custom slash command under `~/.claude/commands/` — the skill's `description` frontmatter already triggers on the slash invocation and the natural-language phrases.
...
- Skill: `.claude/skills/wiki-write/SKILL.md`
- Eval definitions: `.claude/skills/wiki-write/evals/evals.json`
- Operating manual: `docs/llm_wiki/CLAUDE.md` (the contract this skill enforces)
...
- Design plan: `docs/develop/active/meta/llm_wiki_system_design.md`
- Ship commit: `f878873`
...
## Backlinks
- [[20260513_2310_orphan_memory_branch_rewrite]] (wiki_system_design, 2026-05-13) — When a worktree branch carrying a single .claude-memory insight commit falls beh
- [[20260516_1431_v2_three_role_architecture]] (wiki_system_design, 2026-05-16) — Memory System v2 separates code/memory knowledge into three surfaces — curated s
<!-- END BACKLINKS -->
