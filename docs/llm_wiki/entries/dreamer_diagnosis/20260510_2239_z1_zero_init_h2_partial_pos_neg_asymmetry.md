...
- **Promote `zero_init_reward_critic: true` to default**. Both `configs/models/dreamer_v3/dreamer_v3.yaml` and `configs/models/dreamer_v3/dreamer_v3_rr06.yaml` ship with the key set to `true`. The `false` branch is preserved for legacy reproduction.
