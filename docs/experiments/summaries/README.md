...
|---|---|---|---|---|
| 2026-07-29 | 11:00 | [20260729_experiment_history_may_to_jul](20260729_experiment_history_may_to_jul.md) | Experiment history — May–July 2026 (chronological campaign review) | Portfolio-level timeline (not a single-run analysis) of every campaign run **2026-05-01 → 2026-07-29**, sourced from the diary (58 files) + the LLM Wiki. Organised by month, then by campaign, with owning-doc links, wiki-insight links, and model tags. Covers six threads: hypervigilance/danger-discrimination, the NMN/FiLM neuromodulator question, the in-house DreamerV3 (`dreamer_srl`)-vs-recurrent-PPO survival gap, the interoceptive behavior-measure/probe platform, environment/curriculum engineering (basic00–07 ladder), and config-system correctness + cluster/agent tooling. Includes a ~80-tag model-tag index appendix and a gaps section (owning-doc drift, in-flight campaigns with no closing verdict). ~25 campaigns, ~90 links |
| 2026-06-12 | 16:25 | [20260612_1625_predator_rabbit_discrimination](20260612_1625_predator_rabbit_discrimination.md) | Predator vs rabbit danger-discrimination (June thread — pain-reactor vs danger-recogniser) | Does the agent recognise danger or just react to pain? Four June controls (chasing-rabbit, matched-aggression, predator-only, single-pred-vs-rabbit lethal-contact) converge on **pain-consequence-driven avoidance, not anticipatory danger recognition** — agent is class-blind even when predator and rabbit are byte-identical and a single contact can be lethal. Plus the metric-vs-trajectory methodology lesson (aggregates hid a conditional behaviour; raw-obs + step-by-step read exposed it). Single seed (42), recurrent-PPO, 10M-ep checkpoints (2026-05-29 → 2026-06-12) |
