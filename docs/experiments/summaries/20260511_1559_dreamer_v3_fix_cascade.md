...
### Sheeprl-comparison candidate ledger (what's been implemented vs what's left)

The cascade is iterating through a fixed set of paper-canonical deviations between our codebase and the published DreamerV3 reference. Items 27–30 came from the sheeprl side-by-side walk in row 1; item 2 came from the math-reviewer's earlier paper-equation audit on 2026-05-09 (during the Phase 4 review of the implementation reference doc) and was *later* triply-confirmed by the sheeprl walk (paper text + Hafner's published code + sheeprl all match each other; our pre-Z2 codebase was the outlier). Item 2's status as "first surfaced by math-reviewer, later confirmed by sheeprl" doesn't change its membership in the same paper-canonical candidate set the cascade is walking through.

| # | What it does in one sentence | Status | Verdict / mechanistic rationale |
|---|---|---|---|
| **#27** | Initialise the reward and critic output layers to **zero** so the heads start predicting nothing-yet instead of random noise (`uniform_init_weights(0.0)` on the final Linear of each head only). | **DONE** — Cell Z1 (2026-05-10) | H2 partial fix: reward MAE 0.39 → 0.28 (−28%). Predicted asymmetry validated — positive-event prediction improved 49%, negatives only 14%. Now project default. |
| **#2** | Construct the **two-hot reward bins** so they span raw rewards `±4.85×10⁸` (paper-canonical) instead of our legacy `±20` — so death-magnitude rewards are representable instead of saturating at the boundary bin. (First surfaced by math-reviewer's paper-equation audit, before the sheeprl walk; sheeprl, paper text, and Hafner's published code all later confirmed the same paper-canonical layout.) | **DONE** — Cell Z2 (2026-05-11) | H2 partial fix: reward MAE 0.28 → 0.18 (cumulative cascade −54% from A1). Predicted asymmetry validated — negative-event MAE dropped 45% Z1→Z2 while positive-event MAE regressed slightly, exactly the curative-on-negatives signature the bin-coverage mechanism predicted. Now project default. |
| **#28** | Apply the GRU's **reset gate** to the candidate hidden-state update (the variable is currently computed via sigmoid but then never multiplied into anything; paper formula is `cand = tanh(reset * cand)`, ours is `cand = tanh(cand)`). | **NEXT in the queue** | Pending. Mechanistic match for the new Z2 residual: reward MAE compounds 0.18 → 3.05 across horizon 5 → horizon 50, which is exactly the failure mode a GRU-cell-dynamics bug produces (small per-step latent drift compounds over autoregressive imagination). |
| **#29** | Add a **second critic-loss term** that pulls the online critic's logits toward its own slow-target EMA copy (in addition to the λ-return regression that already exists). | Deferred | Not yet planned. Affects value-learning stability rather than reward-head prediction; not the most-targeted match for any current residual. Queue position depends on what candidate #28's outcome reveals. |
| **#30** | Give the RSSM's **prior + posterior heads a hidden layer** — currently both are single `Linear` projections; paper + sheeprl use one-hidden-layer MLPs. | Deferred | Not yet planned. Architectural undersizing of the latent encoding heads; affects KL terms and imagined dynamics indirectly. Lower-tier severity in the §6 list. |

**Quick read**: 2 of 5 candidates have shipped (both partial wins); 1 is mechanistically queued next; 2 are documented but not on the active path. All cascade work to date has been on the no-predator task — the cumulative fix stack has not yet been tested on the original hypervigilance task.
...
- [20260511_1534_z2_paper_bins_h2_partial_cascade_closure_attempt](../../../docs/llm_wiki/entries/dreamer_diagnosis/20260511_1534_z2_paper_bins_h2_partial_cascade_closure_attempt.md) — Experiment 3 verdict + the long-horizon-compounding residual that names candidate #1 as next.
- [20260511_1535_encode_decode_flag_mismatch_silent_class_bug](../../../docs/llm_wiki/entries/cluster_ops/20260511_1535_encode_decode_flag_mismatch_silent_class_bug.md) — the second instance of the auxiliary-tooling-falls-out-of-sync bug class + the cross-check anti-silent-failure-mode trip-wire.

From prior window (the immediate predecessor):

- [20260510_2239_z1_zero_init_h2_partial_pos_neg_asymmetry](../../../docs/llm_wiki/entries/dreamer_diagnosis/20260510_2239_z1_zero_init_h2_partial_pos_neg_asymmetry.md) — Experiment 2 verdict + the pos/neg asymmetry that picked Experiment 3.
...
- [20260510_2241_residual_error_pattern_directs_next_fix](../../../docs/llm_wiki/entries/subagent_engineering/20260510_2241_residual_error_pattern_directs_next_fix.md) — the methodology rule applied twice so far in the cascade.

Older anchors:

- [20260509_1534_wm_reward_head_localized_failure_a1](../../../docs/llm_wiki/entries/dreamer_diagnosis/20260509_1534_wm_reward_head_localized_failure_a1.md), [20260509_1535_conventional_fixes_battery_verdict_predator_refute](../../../docs/llm_wiki/entries/dreamer_diagnosis/20260509_1535_conventional_fixes_battery_verdict_predator_refute.md), [20260509_1536_train_py_checkpoint_restore_nnx_skew](../../../docs/llm_wiki/entries/cluster_ops/20260509_1536_train_py_checkpoint_restore_nnx_skew.md) — the first instance of the silent class-bug pattern that returned this window.
...
2. Memory insight [20260511_1534_z2_paper_bins_h2_partial_cascade_closure_attempt](../../../docs/llm_wiki/entries/dreamer_diagnosis/20260511_1534_z2_paper_bins_h2_partial_cascade_closure_attempt.md) — Z2 verdict with the load-bearing residual-pattern reasoning that named candidate #1 (3 min).
...
- Memory insight [20260511_1535_encode_decode_flag_mismatch_silent_class_bug](../../../docs/llm_wiki/entries/cluster_ops/20260511_1535_encode_decode_flag_mismatch_silent_class_bug.md) — the defensive practice this window codified (cross-check diagnostic against training-time logger).
