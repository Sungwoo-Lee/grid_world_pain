...
3. **(third priority) Per-action `cont` probe + reward-bin histograms on the predator task** with both knobs in place. The H₁(A1)/H₀(A2) row's standard recommendation. Targets the "structural mismatch" hypothesis at the WM-internal level — does the cont head fail to predict per-action terminations (the imagined-deaths-undifferentiated-per-action hypothesis from `.claude-memory/memories/dreamer_diagnosis/20260508_1432_probe_refutes_imagined_death_absence.md`)?
