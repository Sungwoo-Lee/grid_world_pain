...
Add `"FiLM"` and `"FiLMNoNorm"` as new `modulation.type` options alongside the existing `"Multiplicative"` and `"PreActivation"`. Both FiLM modes reuse the existing modulator head infrastructure (γ and β heads) but change:

1. **γ is unconstrained** — no sigmoid applied to `z_unimodal` / `z_multimodal`
2. **γ head bias init** — changed to 1.0 (identity) instead of 3.0 (sigmoid-dependent)
3. **LayerNorm layers** — added at injection points (FiLM only, not FiLMNoNorm)
4. **Encoder forward path** — new branch for FiLM/FiLMNoNorm modulation types

Both FiLM modes construct the additive heads (β), same as PreActivation.
...
**Goal**: Both FiLM modes construct additive heads (same as PreActivation). The γ bias init differs.

##### Lines 59–121 — `__init__`

The FiLM modes need the same heads as PreActivation (both γ and β). The key difference is **γ bias initialization**: FiLM uses `1.0` (identity in linear space) instead of `3.0` (pass-through in sigmoid space).
...
# AFTER:
self.modulation_type = modulation_type
self.is_film = modulation_type in ("FiLM", "FiLMNoNorm")
```
...
# AFTER:
# For FiLM modes: γ bias = 1.0 (identity scaling); for others: use percept_bias_init (sigmoid domain)
_gamma_bias = 1.0 if self.is_film else percept_bias_init
self.head_unimodal = nnx.Linear(mod_hidden_size, self.num_groups_unimodal,
                               bias_init=nnx.initializers.constant(_gamma_bias), rngs=rngs)
if self.modulation_type == "PreActivation" or self.is_film:
    self.head_unimodal_add = nnx.Linear(mod_hidden_size, self.num_groups_unimodal,
                                       bias_init=nnx.initializers.constant(percept_add_bias_init), rngs=rngs)
```

Apply the same pattern to `head_multimodal` and `head_multimodal_add` (lines 98-102):
...
And the baseline parameters (lines 119-121):
...
**No changes** to `__call__` / `_get_signal` — these are generic and already handle the presence/absence of additive heads via `getattr`.

##### DreamerNeuromodulatorRNN (lines 195–365)

Apply the **exact same pattern** to the Dreamer variant:
- Add `self.is_film = modulation_type in ("FiLM", "FiLMNoNorm")` after line 240
- Change `if self.modulation_type == "PreActivation":` → `if self.modulation_type == "PreActivation" or self.is_film:` at lines 258, 265, 288
- Use `_gamma_bias = 1.0 if self.is_film else percept_bias_init` for head initialization at lines 256-257, 263-264

#### 2. `src/models/recurrent_ppo_network.py` — ObservationEncoder

**Goal**: Add LayerNorm layers for FiLM mode, add FiLM forward path in `forward_with_modulation`.

##### Lines 72–98 — `ObservationEncoder.__init__`

Add LayerNorm layers, constructed only for FiLM mode:

```python
# AFTER line 97 (after self.multimodal_hub = MLP(...)):

# FiLM LayerNorm layers (only constructed for FiLM modulation)
# These are set externally after construction since the encoder doesn't know modulation_type
self.film_unimodal_ln = None   # Set by ActorCriticRNN if FiLM
self.film_multimodal_ln = None # Set by ActorCriticRNN if FiLM
```

**Actually — better approach**: Since the encoder doesn't receive `modulation_type` at construction, and adding it would change the constructor signature, have the `ActorCriticRNN` attach the LayerNorm layers to the encoder after construction. This keeps the change minimal.

##### Lines 154–226 — `ActorCriticRNN.__init__`

After the encoder is constructed (line 183), add LayerNorm layers if FiLM:

```python
# AFTER line 183 (after self.obs_encoder = ObservationEncoder(...)):

# FiLM-specific: attach LayerNorm layers to the encoder
...
Add FiLM and FiLMNoNorm branches. The key difference from PreActivation: **no sigmoid on γ**.
...
def forward_with_modulation(self, x, mod_output, modulation_type: str,
    ...
    Modulation types:
    - "Multiplicative": relu(Wx) · sigmoid(z_gamma)
    - "PreActivation":  relu(sigmoid(z_gamma) · Wx + z_beta)
    - "FiLM":           relu(z_gamma · LayerNorm(Wx) + z_beta)  [γ unconstrained]
    - "FiLMNoNorm":     relu(z_gamma · Wx + z_beta)             [γ unconstrained]
    """
    is_film = modulation_type in ("FiLM", "FiLMNoNorm")

    if self.mode != 'hierarchical':
        x_proj = self.monolith(x)
        if is_film:
            gamma = mod_output.z_unimodal       # unconstrained γ
            beta = mod_output.z_unimodal_add
            if modulation_type == "FiLM" and film_flat_ln is not None:
                x_proj = film_flat_ln(x_proj)
            return jax.nn.relu(gamma * x_proj + beta)
        elif modulation_type == "PreActivation":
    ...
    if is_film:
        gamma1 = mod_output.z_unimodal              # unconstrained
        beta1 = mod_output.z_unimodal_add
        if modulation_type == "FiLM" and film_unimodal_ln is not None:
            # Apply LayerNorm over last dim for each modality independently
    ...
    mm_latent = self.multimodal_hub(mm_in)           # (batch, 128)

    if is_film:
        gamma2 = mod_output.z_multimodal             # unconstrained
        beta2 = mod_output.z_multimodal_add
        if modulation_type == "FiLM" and film_multimodal_ln is not None:
...
Update the call to pass LayerNorm layers:
...
Apply the same pattern as the RecurrentPPO encoder. The Dreamer encoder uses SiLU instead of ReLU.

##### `DreamerObservationEncoder.forward_with_modulation` (lines 259–282)

Same logic: add FiLM/FiLMNoNorm branches with unconstrained γ and optional LayerNorm. Replace `jax.nn.sigmoid(mod_output.z_unimodal)` with raw `mod_output.z_unimodal` for FiLM modes.

```python
# In forward_with_modulation, add FiLM branches:
def forward_with_modulation(self, x, mod_output, modulation_type: str,
                            film_unimodal_ln=None, film_multimodal_ln=None):
    # ... (existing setup code for x_padded) ...
    ...
    if is_film:
        ...
        if modulation_type == "FiLM" and film_unimodal_ln is not None:
            encoded_all = jax.vmap(film_unimodal_ln, in_axes=-2, out_axes=-2)(encoded_all)
        encoded_all = SiLU()(gamma1[..., None, :] * encoded_all + beta1[..., None, :])
    else:
        gamma1 = jax.nn.sigmoid(mod_output.z_unimodal)
        beta1 = mod_output.z_unimodal_add
...
Same pattern for the flat encoder fallback.

##### DreamerV3 WorldModel construction

The WorldModel constructor (wherever it creates the encoder) needs to attach `film_unimodal_ln` and `film_multimodal_ln` to the encoder when `modulation_type == "FiLM"`, same as `ActorCriticRNN` does.

#### 4. `configs/models/neuromodulated_ppo.yaml`

Add FiLM-specific config documentation:
...
No new config keys are needed — the existing `percept_bias_init` and `percept_add_bias_init` are overridden internally for FiLM modes (γ bias forced to 1.0). This prevents accidental misconfiguration.

#### 5. Logging — Metric Adaptation

The existing DreamerV3 trainer logs `mod_z_unimodal_mean` as `jnp.mean(jax.nn.sigmoid(mod_outputs_T.z_unimodal))`. For FiLM modes, sigmoid is not applied — the raw γ value should be logged instead.

##### `src/models/dreamer_v3_trainer.py` (lines 274–286)
...
# AFTER:
# For FiLM modes, γ is already the effective gate value (unconstrained).
# For Multiplicative/PreActivation, apply sigmoid to get the effective gate value.
_apply_gate = (lambda z: z) if wm.modulation_type in ("FiLM", "FiLMNoNorm") else jax.nn.sigmoid
'mod_z_unimodal_mean': jnp.mean(_apply_gate(mod_outputs_T.z_unimodal)),
'mod_z_unimodal_std': jnp.std(_apply_gate(mod_outputs_T.z_unimodal)),
'mod_z_multimodal_mean': jnp.mean(_apply_gate(mod_outputs_T.z_multimodal)),
'mod_z_multimodal_std': jnp.std(_apply_gate(mod_outputs_T.z_multimodal)),
```

The RecurrentPPO trainer does not currently log per-modulator metrics (only `mod_grad_norm`), so no changes needed there.
...
```python
# Apply LN to each of the 9 modalities independently
# encoded_all shape: (batch, 9, 128)
# vmap over the modality axis (axis -2)
encoded_all = jax.vmap(film_unimodal_ln, in_axes=-2, out_axes=-2)(encoded_all)
```

This vmaps the LayerNorm over the 9-modality dimension, applying it to each `(batch, 128)` slice. The same LayerNorm parameters are shared across all 9 modalities — this is intentional, matching FiLM's design where the normalization statistics are per-feature, not per-conditioning-input.

**Note on vmap + nnx.LayerNorm**: `nnx.LayerNorm` in Flax NNX is a stateful module (has learnable scale/bias parameters). Vmapping it over the modality axis is valid because the parameters are shared — vmap broadcasts them. The normalization statistics (mean, var) are computed per-modality per-batch-element, which is the desired behavior.

If vmap over `nnx.LayerNorm` proves problematic at JIT time, the fallback is to reshape `(batch, 9, 128)` → `(batch*9, 128)`, apply LayerNorm, reshape back. This is semantically identical.

### Summary of Files Changed

| File | Change | New Lines (approx) |
|------|--------|-------------------|
| `src/models/neuromodulator.py` | `is_film` flag, conditional head construction, γ bias override | ~15 |
| `src/models/recurrent_ppo_network.py` | FiLM/FiLMNoNorm branches in encoder, LayerNorm construction in ActorCriticRNN, updated call site | ~50 |
| `src/models/dreamer_v3_nnx.py` | Same FiLM branches in DreamerObservationEncoder + Encoder, LayerNorm construction | ~50 |
| `src/models/dreamer_v3_trainer.py` | Conditional sigmoid vs identity for metric logging | ~5 |
